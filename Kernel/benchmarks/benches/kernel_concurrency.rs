//! `benchmark-concurrency-check` が実行する concurrency lane の Criterion harness です。

use arcrtc_benchmarks::{benchmark_lane_window, cases_for_lane, BenchmarkLane};
use criterion::{black_box, criterion_group, criterion_main, Criterion};
use std::{thread, time::Instant};

fn kernel_concurrency(c: &mut Criterion) {
    let window = benchmark_lane_window(BenchmarkLane::Concurrency)
        .unwrap_or_else(|error| panic!("benchmark concurrency window rejected: {error:?}"));
    let cases = cases_for_lane(BenchmarkLane::Concurrency);
    assert_eq!(cases.len(), window.case_count());

    let mut group = c.benchmark_group("kernel_resident_runtime_concurrency");
...
criterion_group!(benches, kernel_concurrency);
