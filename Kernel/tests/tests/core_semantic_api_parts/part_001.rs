...
use arcrtc_core_command::{
    ...
    DecisionReason, DecisionShapeError, IdempotencyClass,
...
fn correlation() -> CorrelationId {
    CorrelationId::new(reference("corr-core-api"))
}
...
fn decision(
    ...
    UseCaseDecision::new(UseCaseDecisionInput {
        ...
        command_type: CommandType::new("core-api-command"),
        target_surface,
        outcome,
        reason,
        state_transition: StateTransitionSummary::Changed("core-api-transition"),
        port_intents: vec![PortIntent::new("core-api-port-intent", target_surface)],
        audit_projection: AuditProjectionRequirement::Required,
    })
}

#[test]
fn command_decision_event_result_shape_preserves_correlation_and_reason() {
    let envelope = CommandEnvelope::new(
        correlation(),
        CommandType::new("join_room"),
        CommandVersion::new(1),
        TargetSurface::Signaling,
        SignalingSubject::new(RoomId::new(reference("room-core-api")), None),
    );
    assert_eq!(envelope.correlation_id().as_str(), "corr-core-api");
    ...
    assert_eq!(rejected.correlation_id().as_str(), "corr-core-api");
    assert!(matches!(rejected.reason(), DecisionReason::Cataloged(_)));
}

#[test]
fn reason_catalog_is_closed_and_decision_reason_rule_is_fail_closed() {
    assert_eq!(
...
#[test]
fn signaling_sfu_turn_decisions_reject_wrong_target_surface() {
...
#[test]
fn turn_transport_and_cross_plane_fail_closed_inputs_are_public_api_checked() {
    assert_eq!(
    ...
    let transaction = TurnTransactionId::new(reference("turn-tx-core-api"));
    ...
    let session_id = SessionId::new(reference("session-core-api"));
    assert_eq!(
    ...
    let policy = CrossPlaneBindingPolicy::new(
        ...
        CrossPlane::Sfu,
        CrossPlaneReference::Participant(ParticipantId::new(reference("participant-core-api"))),
...
#[test]
fn runtime_state_recovery_quality_and_audit_fail_closed_inputs_are_checked() {
    assert_eq!(
    ...
    assert_eq!(
        RuntimeTaskLifecycleDecision::try_new(
            StartupRunId::new(reference("startup-core-api")),
            Some(correlation()),
            policy,
            Some("task-ref-core-api"),
    ...
    let success_with_reason = AuditEvent::new(AuditEventInput {
        event_id: AuditEventId::new(reference("audit-core-api")),
...
#[test]
fn protocol_idempotency_and_canonical_serialization_are_individually_fail_closed() {
