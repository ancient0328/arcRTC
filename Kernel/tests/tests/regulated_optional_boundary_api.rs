...
use arcrtc_kernel_test_suite::{assert_not_contains, read_impl};
...
fn valid_input() -> RegulatedEnrichmentInput {
    RegulatedEnrichmentInput::new(
        ...
        RegulatedCommunicationReference::Correlation(CorrelationId::new(reference(
            "corr-regulated-boundary",
        ))),
        Some(AuditEventId::new(reference("audit-regulated-boundary"))),
...
#[test]
fn regulated_enrichment_admits_only_post_core_optional_opaque_references() {
    ...
    let hash_pointer =
        RegulatedHashChainRecordPointer::accept("hash-chain-record-regulated-boundary")
            .expect("valid pointer");
    assert_eq!(
        hash_pointer.as_str(),
        "hash-chain-record-regulated-boundary"
    );
    assert_eq!(
    ...
    let room_pointer =
        RegulatedCommunicationReference::Room(RoomId::new(reference("room-regulated-boundary")));
    assert!(room_pointer.is_immutable_pointer_only());
}

#[test]
fn regulated_boundary_rejects_core_decision_audit_mutation_and_payload_requirement() {
    let requires_core_decision = RegulatedEnrichmentInput::new(
        RegulatedOptionalSupportClass::DomainSpecificEnrichment,
        RegulatedEnrichmentLifecycleStage::MappedOutsideGenericCore,
        RegulatedCommunicationReference::Correlation(CorrelationId::new(reference(
            "corr-regulated-boundary-decision",
        ))),
        None,
        None,
    ...
    let mutates_core_audit = RegulatedEnrichmentInput::new(
        ...
        RegulatedCommunicationReference::AuditEvent(AuditEventId::new(reference(
            "audit-regulated-boundary-mutate",
        ))),
        None,
    ...
    let requires_domain_payload = RegulatedEnrichmentInput::new(
        ...
        RegulatedCommunicationReference::Correlation(CorrelationId::new(reference(
            "corr-regulated-boundary-domain",
        ))),
        None,
...
#[test]
fn regulated_dependency_direction_is_single_allowed_edge() {
