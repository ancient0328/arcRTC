...
use arcrtc_driver_persistence::{
    ...
    ArtifactRestoreImportGuard, ArtifactRestoreImportGuardError, MigrationCompatibilityRule,
...
#[test]
fn schema_migration_and_restore_policy_are_explicitly_guarded() {
    assert_eq!(
...
#[test]
fn secret_rotation_and_platform_lifecycle_mapping_are_fail_closed() {
    assert_eq!(
...
#[test]
fn turn_wire_decode_projects_only_core_turn_command_fields() {
    assert_eq!(
    ...
    let malformed = TurnWireDecodeInput::new(
        ...
        TurnWireDecodedAttributes::new(
            Some(UntrustedReference::new("turn-tx-driver-api-invalid")),
            None,
            None,
            None,
            None,
    ...
    let allocate = TurnWireDecodeInput::new(
        ...
        Some(TurnWireMethodClass::AllocateRequest),
        TurnWireDecodedAttributes::new(
            Some(UntrustedReference::new("turn-tx-driver-api-valid")),
            None,
            None,
            None,
            None,
...
#[test]
fn driver_conversion_returns_core_owned_command_or_closed_failure() {
    let input = DriverCommandConversionInput::new(
        valid_preconditions(),
        valid_delegation(),
        true,
        Some(UntrustedReference::new("corr-driver-api")),
    ...
    assert_eq!(envelope.correlation_id().as_str(), "corr-driver-api");
...
#[test]
fn driver_rejects_semantic_decision_ownership_and_wire_encode_fail_open() {
    assert_eq!(
    ...
    let correlation = arcrtc_core_identity::CorrelationId::new(
        OpaqueReference::accept("corr-driver-api-encode", ReferenceAuthority::CorePolicy)
...
#[test]
fn external_error_projection_preserves_core_reason_metadata() {
    let unsafe_reason =
    ...
    let opaque_error_ref =
        OpaqueReference::accept("opaque-error-driver-api", ReferenceAuthority::CorePolicy).unwrap();
    ExternalErrorProjection::try_new(ExternalErrorProjectionInput {
        surface: ExternalErrorSurface::Http,
        wrapper_class: ExternalStatusWrapperClass::HttpStatusWrapper,
        correlation_id: None,
...
#[test]
fn platform_drivers_do_not_claim_domain_semantic_ownership() {
    for path in [
...
#[test]
fn concrete_driver_guards_fail_closed_across_platform_storage_security_and_observability() {
    assert_eq!(
