#[test]
fn ce2_sfu_packet_transform_negotiation_and_congestion_contracts_are_core_owned() {
    let packet_id = PacketId::new(reference("packet-ce2-sfu"));
    let stream_id = StreamId::new(reference("stream-ce2-sfu"));
    let endpoint_id = EndpointId::new(reference("endpoint-ce2-sfu"));
    ...
    let transform = PacketRewriteTransformIntent::new(
        RouteId::new(reference("route-ce2-transform")),
        Some(endpoint_id),
...
#[test]
fn ce2_secure_media_token_and_clock_skew_boundaries_reject_untrusted_inputs() {
    let credential_ref = CredentialRef::new(reference("credential-ce2"));
    ...
    assert_eq!(
        TimeSynchronizationDecision::try_new(
            StartupRunId::new(reference("startup-time-ce2")),
            Some(correlation()),
            policy,
