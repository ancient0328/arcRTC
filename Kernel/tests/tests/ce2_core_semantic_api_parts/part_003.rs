...
#[test]
fn ce2_cov1_recovery_restore_replay_and_failover_paths_are_fail_closed() {
    assert!(
    ...
    assert!(
        !recovery::StateFamilyRecoveryPolicy::canonical_default(StateFamily::SignalingRoom)
    ...
    assert_eq!(
        recovery::ReplayPolicyCoverage::try_new(false, true, true, true, true, true, true, true),
    ...
    let policy = recovery::DistributedStatePolicy::try_new(
        ...
        recovery::RecoveryRestoreRelation::ExplicitRestoreEvidenceRequired,
    ...
    let failover_shape = recovery::FailoverEvidenceShape::try_new(failover_input(Some(
        reference("replacement-cov1"),
    )))
    .expect("evidence-backed failover has all required fields");
    assert_eq!(
        recovery::FailoverEvidenceShape::try_new(failover_input(None)),
        Err(recovery::FailoverEvidenceShapeError::ReplacementOwnerMissing)
    );
    ...
    let crash_evidence = recovery::CrashRestartEvidenceShape::try_new(
        "kill -ABRT arcrtc",
        "supervisor-cov1",
        recovery::ProcessFailureClass::ProcessCrashObserved,
        recovery::ProcessFailureClass::ProcessCrashObserved,
    ...
    .expect("crash evidence separates logs, audit, restore, and readiness");
    assert_eq!(
        ...
        Err(recovery::CrashRestartEvidenceShapeError::CommandOrProcedureMissing)
    );
    assert_eq!(
        recovery::RestartDomainStateClaim::try_new(classification, None, Some(crash_evidence.clone())),
...
#[test]
fn ce2_cov1_operation_ordering_retry_and_atomicity_boundaries_are_explicit() {
    assert_eq!(
    ...
    assert_eq!(
        operation::DrainSequenceStep::StopRuntimeAfterEvidencePathAttempt.order(),
        10
    );
    assert_eq!(
    ...
    let _drain_evidence = operation::ShutdownDrainEvidenceShape::new(
        correlation(),
        Some(StartupRunId::new(reference("startup-drain-cov1"))),
        operation::ShutdownDrainMode::GracefulDrain,
        None,
        operation::ShutdownDrainPlane::Signaling,
        operation::ShutdownDrainOutcome::Rejected,
        Some(operation::ShutdownDrainFailureKind::RoomDraining),
        true,
        true,
        Some(0),
    );
    ...
    assert!(!operation::RetryClass::TestOnlyRetry.runtime_evidence_allowed());
    assert_eq!(
    ...
    assert_eq!(
        ...
        operation::CancellationHandling::PreservePriorDecisionEvidence
    );
...
#[test]
fn ce2_cov1_quality_resource_and_backpressure_catalogs_are_asserted() {
...
#[test]
fn ce2_cov1_ports_contract_persistence_and_failure_mappings_are_core_owned() {
    let context = ports::PortCallContext::new(correlation());
    assert_eq!(context.correlation_id().as_str(), "corr-ce2");
