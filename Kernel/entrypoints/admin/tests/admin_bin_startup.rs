...
fn assert_case(args: &[&str], stdout: &str, stderr: &str, code: i32) {
    let output = Command::new(env!("CARGO_BIN_EXE_arcrtc-entrypoint-admin"))
        .args(args)
        .output()
        .expect("admin entrypoint must execute");
...
#[test]
fn admin_bin_startup_cases_match_fixed_surface() {
    assert_case(&["process-liveness"], "probe_class=process-liveness", "", 0);
