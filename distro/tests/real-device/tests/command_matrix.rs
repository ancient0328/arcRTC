...
fn base(exit_status: i32) -> DistroEvidenceRecord {
    let (actual_outcome, distro_reason) = match exit_status {
        0 => (
...
fn browser_record(exit_status: i32) -> RealDeviceEvidenceRecord {
    RealDeviceEvidenceRecord {
        ...
        logs_metrics_location: format!("{DISTRO_EVIDENCE_ROOT}/real-device/browser.json"),
...
fn android_record(exit_status: i32) -> RealDeviceEvidenceRecord {
    RealDeviceEvidenceRecord {
        ...
        logs_metrics_location: format!("{DISTRO_EVIDENCE_ROOT}/real-device/android.json"),
        redacted_device_identifier: (exit_status == 0).then(|| "redacted".to_owned()),
    }
}
...
fn kpi_real_device_command_matrix_requires_bounded_evidence_fields() {
    for record in [
        ...
        assert!(record
            ...
            .starts_with(&format!("{DISTRO_EVIDENCE_ROOT}/real-device/")));
...
fn real_device_evidence_rejects_context_mismatches() {
    ...
    wrong_command_class.base.command_class = DistroCommandClass::Build;
    wrong_command_class
        .base
        .non_claim_scope
        .push(DistroNonClaimScope::BehaviorCorrectnessNotClaimed);
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(RealDeviceEvidenceValidationError::Base(
            arcrtc_distro_evidence::EvidenceValidationError::MissingRequiredNonClaimScope
        ))
    );
    ...
    missing_kernel_non_claim
        .base
        .non_claim_scope
        .retain(|scope| *scope != DistroNonClaimScope::KernelCompletionNotClaimed);
    assert_eq!(
...
fn real_device_evidence_rejects_post_execution_field_violations() {
    ...
    assert_eq!(
        ...
        Err(RealDeviceEvidenceValidationError::Base(
            arcrtc_distro_evidence::EvidenceValidationError::MissingRequiredExitStatus
        ))
    );
