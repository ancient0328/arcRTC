...
use wrapper::{
    ...
    distro_reason_for_exit, planned_exit_for_dispatch, preflight_outcome_for_dispatch,
...
fn base_evidence(command_class: DistroCommandClass) -> DistroEvidenceRecord {
    DistroEvidenceRecord {
        ...
        environment_class: DistroEnvironmentClass::LocalSingleHost,
...
fn real_device_command_output_covers_status_reason_and_summary_branches() {
    for (status, code, reason, is_success) in [
        ...
        assert_eq!(status.distro_reason(), reason);
...
fn real_device_evidence_validation_covers_context_and_command_branches() {
    ...
    wrong_command.base.command_class = DistroCommandClass::Test;
    assert_eq!(
...
fn real_device_wrapper_success_record_covers_success_outcome_matrix() {
    for (platform, device_class, output, observation_expected) in [
        ...
        assert_eq!(
            ...
            output.exit_status.distro_reason()
        );
        assert_eq!(
...
fn real_device_base_evidence_validation_covers_shared_fail_closed_branches() {
    for (record, expected) in [
        {
            let mut record = base_evidence(DistroCommandClass::RealDevice);
