...
use wrapper::{
    ...
    is_current_working_directory_distro_root, RealDeviceWrapperExit,
};

fn main() {
    debug_assert_eq!(all_wrapper_exit_codes(), [0, 2, 3, 4, 5]);
    if !is_current_working_directory_distro_root() {
...
mod tests {
    ...
    use super::{
        ...
        wrapper::{
            actual_outcome_for_exit, build_real_device_record, distro_reason_for_exit,
    ...
    fn unit_evidence_validation_covers_context_and_post_execution_matrix() {
        ...
        record.base.command_class = DistroCommandClass::Test;
        assert_eq!(
    ...
    fn unit_wrapper_orchestration_covers_scope_and_profile_fail_closed() {
        ...
        std::env::set_current_dir(distro_root()).expect("distro cwd");
        let cli = RealDeviceCli {
            platform: CliPlatform::Browser,
            profile: REAL_DEVICE_PROFILE_REFERENCE_LOCAL.to_owned(),
            device_class: CliDeviceClass::DesktopBrowser,
        ...
        assert!(record
            .base
            .non_claim_scope
            .contains(&DistroNonClaimScope::KernelCompletionNotClaimed));
    }
}
