...
use arcrtc_reference_ops::{
    build_reference_build_evidence_record, DistroShutdownMode, ReferenceBuildEvidenceInput,
    ReferenceRuntime,
};
...
fn product_policy_evaluate() -> usize {
    let mut completed = 0;
    for (plane, action) in [
        (DistroPlane::Signaling, ProductAction::Join),
        (DistroPlane::Turn, ProductAction::Relay),
        (DistroPlane::Sfu, ProductAction::Publish),
    ] {
...
fn product_runtime_select() -> usize {
    ...
    usize::from(selection.distro_reason == DistroEvidenceReason::DistroOk)
}
