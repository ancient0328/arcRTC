...
This chapter fixes, at a granularity sufficient for reproducible re-implementation, the phase / fixture payload (all phases and payloads), state mutation rules (all state transitions), reference-local API shape and state types, output boundary (the 3-type allow-list the product can consume), runtime lifecycle (all states), and command evidence runner (execution and evidence generation procedure) of the reference distro within the arcRTC v0.2 distro area. This chapter is fully self-contained and is understandable without consulting other documents or source code. Only other chapter numbers within this same specification are referenced.
...
`reference-distro/signaling/src/state.rs` holds the following local phase enums.
...
`reference-distro/signaling/src/fixture_identity.rs` holds the following.
...
`reference-distro/turn/src/fixture_credential.rs` is fixed to:
...
`reference-distro/sfu/src/fixture_route_auth.rs` is fixed to:
...
`reference-distro/sfu/src/state.rs` owns the following local phase enums.
...
`ReferenceSfuAction` is the only local input enum allowed to mutate SFU reference state. These local SFU state enums mirror the frozen Kernel state variant names and MUST NOT introduce distro-only variants.
...
This section fixes the reference-local API, state fields, state transitions, and Kernel contract builder mapping. This chapter does not redefine Kernel semantics. distro-local types are limited to input, projection, and evidence for building the Kernel public contract.
...
|---|---|---|
| input constructor | `Reference{Plane}Input::new(...)` | distro-local input |
| Kernel contract builder | `build_kernel_{plane}_command(...)` or `build_kernel_{plane}_item(...)` | Kernel public contract type |
| state application | `apply_reference_{plane}(...)` | distro-local outcome |
| event / evidence projection | `project_reference_{plane}_event(...)` | Kernel event or evidence record |
| validation | `validate_reference_{plane}_state(...)` | distro-local validation result |

A Kernel contract object that does not expose payload accessors MUST NOT be treated as a local source of truth. The original distro-local input remains the source for reference behavior application.

### 3.2 Product input allow-list boundary

The reference types the product distro can consume are limited to `ReferenceSignalingOutcome`, `ReferenceTurnOutcome`, and `ReferenceSfuOutcome` owned by `arcrtc-reference-output`.
...
|---|---|---|
| `ReferenceSignalingCommandInput` | `reference-distro/signaling/src/kernel_contract.rs` | `correlation_id: CorrelationId`, `room_id: RoomId`, `participant_id: Option<ParticipantId>`, `kind: SignalingCommandKind`, `payload: ReferenceSignalingPayload` |
| `ReferenceSignalingPayload` | `reference-distro/signaling/src/kernel_contract.rs` | enum variants of section 1.3 |
| `ReferenceSignalingState` | `reference-distro/signaling/src/state.rs` | `rooms: BTreeMap<String, ReferenceRoomState>`, `participants: BTreeMap<String, ReferenceParticipantState>` |
| `ReferenceRoomState` | `reference-distro/signaling/src/state.rs` | `room_id: RoomId`, `phase: ReferenceRoomPhase`, `participant_ids: BTreeSet<String>` |
| `ReferenceParticipantState` | `reference-distro/signaling/src/state.rs` | `participant_id: ParticipantId`, `room_id: RoomId`, `phase: ReferenceParticipantPhase` |
| `ReferenceSignalingOutcome` | `reference-distro/output/src/signaling.rs` | `correlation_id: CorrelationId`, `kind: SignalingEventKind`, `room_id: RoomId`, `participant_id: Option<ParticipantId>`, `distro_reason: DistroEvidenceReason` |
...
| Type | File | Required fields |
|---|---|---|
| `ReferenceTurnCommandInput` | `reference-distro/turn/src/kernel_contract.rs` | `kind: TurnCommandKind`, `transaction_id: TurnTransactionId`, `references: TurnReferenceSet`, `allocation_id: Option<AllocationId>`, `permission_id: Option<PermissionId>`, `channel_bind_id: Option<ChannelBindId>`, `credential_ref: Option<CredentialRef>`, `peer_address: Option<CorePeerAddress>`, `requested_lifetime: Option<TurnRequestedLifetimeSeconds>`, `relay_packet_id: Option<PacketId>` |
| `ReferenceTurnState` | `reference-distro/turn/src/state.rs` | `allocations: BTreeMap<String, ReferenceAllocationState>`, `permissions: BTreeMap<String, ReferencePermissionState>`, `channel_binds: BTreeMap<String, ReferenceChannelBindState>` |
| `ReferenceAllocationState` | `reference-distro/turn/src/state.rs` | `allocation_id: AllocationId`, `phase: AllocationState`, `credential_ref: Option<CredentialRef>` |
| `ReferencePermissionState` | `reference-distro/turn/src/state.rs` | `permission_id: PermissionId`, `allocation_id: AllocationId`, `peer_address: CorePeerAddress`, `phase: PermissionState` |
| `ReferenceChannelBindState` | `reference-distro/turn/src/state.rs` | `channel_bind_id: ChannelBindId`, `permission_id: PermissionId`, `phase: ChannelBindState` |
| `ReferenceTurnOutcome` | `reference-distro/output/src/turn.rs` | `kind: TurnDecisionKind`, `distro_reason: DistroEvidenceReason` |
...
| Type | File | Required fields |
|---|---|---|
| `ReferenceSfuContractInput<Payload>` | `reference-distro/sfu/src/kernel_contract.rs` | `model_kind: SfuModelKind`, `references: SfuReferenceSet`, `payload: Payload` |
| `ReferenceSfuAction` | `reference-distro/sfu/src/state.rs` | enum variants listed below |
| `ReferenceSfuSuppressionSource` | `reference-distro/sfu/src/state.rs` | enum variants `Backpressure`, `Quality` |
| `ReferenceSfuState` | `reference-distro/sfu/src/state.rs` | `sessions: BTreeMap<String, ReferenceSfuSessionState>`, `endpoints: BTreeMap<String, ReferenceEndpointState>`, `routes: BTreeMap<String, ReferenceRouteState>` |
| `ReferenceSfuSessionState` | `reference-distro/sfu/src/state.rs` | `session_id: SessionId`, `phase: SfuSessionState` |
| `ReferenceEndpointState` | `reference-distro/sfu/src/state.rs` | `endpoint_id: EndpointId`, `session_id: SessionId`, `phase: SfuEndpointState` |
| `ReferenceRouteState` | `reference-distro/sfu/src/state.rs` | `route_id: RouteId`, `session_id: SessionId`, `endpoint_id: EndpointId`, `stream_id: StreamId`, `phase: SfuRouteState` |
| `ReferenceSfuOutcome` | `reference-distro/output/src/sfu.rs` | `kind: SfuDecisionKind`, `distro_reason: DistroEvidenceReason` |

`ReferenceSfuAction` is distro-local. It carries the IDs needed by reference state mutation because frozen Kernel `SfuReferenceSet` only exposes `session_id()` publicly. Variants are fixed to:
...
| `apply_reference_sfu` | `(&mut ReferenceSfuState, &ReferenceSfuAction) -> Result<ReferenceSfuOutcome, ReferenceSfuError>` | updates the in-memory routing projection using distro-local action fields |
...
Reference state mutation MUST NOT read endpoint / stream / route / packet fields from Kernel `SfuReferenceSet`. Those fields are not public accessors in the frozen Kernel contract. Reference mutation uses `ReferenceSfuAction` fields as distro-local input and maps only the outcome kind to Kernel `SfuDecisionKind`.
...
| Type | File | Required fields |
|---|---|---|
| `ReferenceCompositionState` | `reference-distro/composition/src/composition_state.rs` | `signaling: ReferenceSignalingState`, `turn: ReferenceTurnState`, `sfu: ReferenceSfuState`, `bindings: BTreeMap<String, ReferenceCrossPlaneBinding>` |
| `ReferenceCrossPlaneBinding` | `reference-distro/composition/src/composition_state.rs` | `correlation_id: CorrelationId`, `room_id: Option<RoomId>`, `session_id: Option<SessionId>`, `allocation_id: Option<AllocationId>`, `route_id: Option<RouteId>` |
| `ReferenceCompositionOutcome` | `reference-distro/output/src/composition.rs` | `correlation_id: CorrelationId`, `distro_reason: DistroEvidenceReason` |
...
Each `Reference*Error` enum MUST map to `DistroEvidenceReason`. No reference error enum may contain `Unknown` (MUST NOT).

### 3.8 Collapse conditions

- A Kernel command private payload is treated as the source of truth for distro behavior.
...
- The Kernel reason catalog is extended with a distro-local reason.
...
This section limits the reference distro surface the product distro can consume to the outcome types of the `arcrtc-reference-output` package. The product distro does not inherit reference source, reference internal state, fixture, local auth, runtime, composition state, or reference success claims.

### 4.1 Package boundary

| Item | Fixed value |
|---|---|
| package path | `reference-distro/output` |
| package name | `arcrtc-reference-output` |
| owner | distro reference-output boundary |
...
This section fixes the lifecycle API for reference runtime, product runtime, startup, shutdown, drain, and correlation propagation. The runtime lifecycle is distro process orchestration and does not own Kernel semantics. A runtime lifecycle outcome is not command evidence. build / test command evidence is generated only by the runner helper of section 6.

### 5.1 Runtime states

| Type | File | Variants |
|---|---|---|
| `DistroRuntimeState` | `reference-distro/ops/src/runtime.rs` and `product-distro/deployment/src/runtime.rs` | `Created`, `Starting`, `Running`, `Draining`, `Stopped`, `Failed` |
| `DistroShutdownMode` | `reference-distro/ops/src/runtime.rs` and `product-distro/rollback/src/drain.rs` | `Immediate`, `GracefulLocal`, `DrainThenStop` |
| `DistroRuntimePlane` | `reference-distro/ops/src/runtime.rs` | `Signaling`, `Turn`, `Sfu`, `Composition` |
| `ReferenceRuntimeOutcome` | `reference-distro/ops/src/runtime.rs` | `correlation_id`, `state`, `distro_reason`, `non_claim_scope` |
...
| `ReferenceRuntime::shutdown` | `(&mut self, CorrelationId, DistroShutdownMode) -> Result<ReferenceRuntimeOutcome, ReferenceRuntimeError>` | transition `Running -> Draining -> Stopped` |
| `ReferenceRuntime::state` | `(&self) -> DistroRuntimeState` | observes local runtime state only |
...
Runtime timeout and cancellation are distro-local execution facts.
...
This section fixes the distro command evidence runner, output path, JSON writer, and validation commands. The command evidence runner only generates evidence records; it does not automatically claim build success, test pass, benchmark pass, or readiness success.
...
The shared evidence record / reason type owner is `arcrtc-distro-evidence`. distro command root / target root / evidence root constants are owned by `distro-support/evidence/src/validation.rs` and re-exported by `arcrtc-distro-evidence`. No runner may write evidence outside `distro/target/` (MUST NOT).

### 6.2 Output path rule

| Evidence class | Output directory |
|---|---|
| format | `target/distro-evidence/format/` |
| build | `target/distro-evidence/build/` |
| test | `target/distro-evidence/test/` |
| benchmark | `target/distro-evidence/benchmark/` |
| real-device | `target/distro-evidence/real-device/` |
| production readiness | `target/distro-evidence/production-readiness/` |
| live readiness | `target/distro-evidence/live-readiness/` |
...
The JSON writer MUST serialize `DistroEvidenceRecord` using snake_case field names and the defined wire values. Required validation before write:

- `validate_evidence_record(&record)` from `arcrtc-distro-evidence` returns `Ok(())`.
...
- The product monitoring evidence helper accepts only `Product` records whose command class is `Build` or `Test`, whose target package is an `arcrtc-product-*` package, and whose target scope is under `product-distro/`.
...
|---|---|---|
| `rg` for placeholder / forbidden tokens (`{PROJECT}` / `{MISSION}` / `TODO` / `TBD` / undefined markers, etc.) | `distro` | no output |
| `rg` for boundary tokens (`v0.2/userland` / `userland` / `adapters/` / `apps/`, etc.) | `distro` | no output |
| `rg` for `UNKNOWN` / `Unknown` / `Other` / `raw String` | `distro` | prohibition text only |
...
- The evidence writer writes outside `target/distro-evidence/`.
...
- The runner adopts a Kernel working directory command result as distro evidence.
