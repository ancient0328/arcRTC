...
This chapter fixes completely how the distro area of arcRTC v0.2 (the non-Kernel distro area) imports the frozen Kernel public contract, pins its version, maps it into reference / product builders, and pins the consumed Kernel source as a frozen read-only input. With the invariant that the distro side MUST NOT own, overwrite, or duplicate Kernel semantic authority, this chapter internalizes the importable surface set, the import path rule, the full builder constructor-chain mappings, and the required inputs and fail-closed conditions of the Kernel source pin. This chapter is self-contained and is written at a granularity sufficient for re-implementation without opening any other document.

## Dependency-direction invariant

distro depends on the Kernel public contract / SDK projection / documented command surface (`distro -> Kernel`). The reverse dependency (`Kernel -> distro`) is forbidden. distro MUST NOT own or overwrite Kernel semantic authority. When a change to the Kernel contract is required, the distro side MUST fail closed, and a formal versioned Kernel-side contract revision MUST come first. distro MUST NOT bypass, duplicate, or extend the Kernel contract on its own side.

## 1. Importable surface

distro MAY use the following Kernel surfaces. This is a closed set; using any surface not listed is forbidden.

| Surface | Allowed use |
|---|---|
| Kernel public contract | input contract for SFU / TURN / Signaling distro |
| Kernel SDK projection | Signaling-side public API projection |
| core-owned port definition | contract for the driver / distro boundary |
...
| Kernel surface | distro-side use |
|---|---|
| public contract | input contract for Signaling / TURN / SFU distro |
| SDK projection | Signaling public projection |
| core-owned port definition | connection contract for distro mapper / runtime wrapper |
| documented command surface | origin of benchmark / test / evidence commands |
| Kernel report reference | basis for non-claim / boundary |

## 2. Non-importable surface

distro MUST NOT import, overwrite, duplicate, or own the following. This is a closed set.
...
distro references the Kernel under the following conditions (MUST).
...
- When a Kernel-side change is required, distro MUST fail closed.

### Upgrade rule

When updating a Kernel dependency, the distro side MUST do the following.

1. Confirm whether a formal versioned Kernel-side contract revision exists.
2. Enumerate the affected distro surfaces.
...
A distro that uses the Kernel contract MUST be designed in the following order.

1. Identify the Kernel surface to be used by its source path.
2. Define the required wrapper / mapper / runtime configuration on the distro side.
...
5. When a Kernel contract gap is found, stop the distro task.
6. A contract for which no formal versioned Kernel-side contract revision exists MUST NOT be alternatively defined on the distro side.

### Allowed wrapper

The wrappers that MAY be placed on the distro side are limited to the following closed set.
...
| error mapper | project Kernel reason into a distro-facing report reason |
| command wrapper | launch assistance for benchmark / test / execution commands |
| observation mapper | convert metrics / log / trace output into the distro evidence shape |

### Prohibited wrapper

The wrappers that MUST NOT be placed on the distro side are the following.
...
- a wrapper that converts Kernel completion evidence into distro readiness proof

## 6. Builder mapping (Kernel public constructor invocation rule)

This fixes the order and arguments with which a reference builder invokes Kernel public constructors. This mapping does not redefine Kernel semantics. The Kernel source MUST NOT be changed, and distro consumes the Kernel public API.
...
- MUST fix command version to `CommandVersion::new(1)` in the initial reference distro.
...
distro pins the consumed Kernel source snapshot as a read-only input. The distro side MUST NOT change the Kernel source, and MUST NOT treat the Kernel source as a substitute evidence for production readiness / live readiness / benchmark threshold satisfaction / real-device success.
...
| `distro_kernel_mutation_absence` | `Kernel source is read-only input for the binding; the binding writes only under distro/` |

### 7.2 Snapshot command

The consumed Kernel source snapshot identifier is computed from the distro root with:
...
The consumed file count is computed from the distro root with:
...
| target package | `arcrtc-distro-boundary-tests` |
...
| reason classification | `DistroOk` |
...
- `Kernel/**/Cargo.toml`
- `Kernel/**/Cargo.lock`
- `distro/tests/boundary/tests/kernel_completion_freeze_binding.rs`

### 7.6 Basis of the pin

The distro side consumes the Kernel source as a read-only, version-pinned input identified by its snapshot digest and file count. The distro side MUST NOT repurpose the consumed Kernel source into distro product completion or readiness evidence; production readiness / live readiness / benchmark threshold satisfaction / real-device success are established by distro-side dedicated evidence, not derived from the Kernel source pin. The correct responsibility of the distro side is limited to consuming the Kernel source without modification and fixing the rerun conditions.

## 8. Fail-closed rule

When a Kernel contract gap is found during distro, it MUST be handled in the following order.

1. Stop the distro-side task.
2. Record the gap in the distro report.
3. State explicitly that a formal versioned Kernel-side contract revision is required.
4. MUST NOT bypass, duplicate, or extend the Kernel contract on the distro side.
...
- The Kernel source is changed during distro-side fixed-goal work.
- The Kernel source pin is used as substitute evidence for production readiness / live readiness / benchmark threshold satisfaction / real-device success / distro product completion.
...
- distro changes Kernel semantic authority.
- A Kernel contract gap is filled with an alternative type on the distro side.
- A Kernel crate not in the dependency matrix is imported.
- Kernel evidence is adopted as distro completion / readiness proof.
- Kernel source modification is treated as an ordinary distro task.
- distro duplicates, extends, or bypasses the Kernel contract.
- A wrapper / mapper owns Kernel semantics.
- A distro task continues when a Kernel contract gap exists.
- distro copies the Kernel source.
