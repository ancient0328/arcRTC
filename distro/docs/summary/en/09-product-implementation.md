...
This chapter fixes, at a granularity sufficient for reproducible re-implementation, the product distro layer (the product distro domain outside the Kernel) of the arcRTC v0.2 distro domain: its owned surfaces, required package shape, construction order, non-inheritance rules from reference, product API policy, the exact API signature of each plane (fully enumerated), product policy decision rules, persistence topology, deployment / runtime profile mapping, production provider admission, live endpoint traversal admission, monitoring / evidence, rollback / drain, fail-closed rules, and readiness boundary. This chapter is fully self-contained and is understandable without consulting other documents or source code. It references only other chapter numbers within this same specification.

product distro is not the Kernel. product distro MAY use the Kernel frozen contract. product distro MUST NOT own Kernel semantics. product distro MUST NOT fork / copy the reference distro. product distro MUST consume only the 3-type allow-list `ReferenceSignalingOutcome` / `ReferenceTurnOutcome` / `ReferenceSfuOutcome` of `arcrtc-reference-output` at the product boundary.

---

## 1. Owned surfaces

product distro MUST own the following surfaces.
...
The decision class product distro MAY own is limited to product policy. The owner relation between Kernel semantics and product policy is fixed as follows.
...
product distro MUST NOT own the following.
...
product distro packages MUST follow the distro workspace members. Each package MUST hold the following file contract.
...
product admission MUST NOT claim the following: reference distro completion / product distro completion / production readiness / live readiness / benchmark threshold satisfaction / public distribution readiness.
...
`build_product_runtime_profile` maps `ProductHostClass` to `ProductRuntimeProfile` fields as follows. `select_product_runtime` then returns the listed `ProductRuntimeSelection.distro_reason`.
...
product distro test MAY demonstrate product behavior. production readiness requires `PRD-001` through `PRD-009` of the readiness matrix (MUST). live readiness requires `LIVE-001` through `LIVE-008` of the readiness matrix and MUST NOT be claimed until the production readiness evidence, live endpoint evidence, public traversal evidence, monitoring probe evidence, rollback / drain execution evidence, shutdown drain evidence, and restore evidence are all in place.

product distro evidence MUST be layered as follows: 1. product behavior evidence, 2. production readiness evidence, 3. live readiness evidence. Lower evidence MUST NOT automatically prove higher readiness. Each readiness claim MUST be established by its own dedicated evidence.

## 18. Fail-closed / collapse conditions

If any of the following holds, the product distro specification collapses (treated as fail-closed and not established).

- product distro includes Kernel source modification as a normal task.
...
- `ProductEvidenceRecord` is defined as a struct independent from the shared `DistroEvidenceRecord`.
...
- a product API redefines `DistroEvidenceReason` as a package-local enum.
