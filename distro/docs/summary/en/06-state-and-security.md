...
1. What distro persists and what it does not persist, and what the source-of-truth rule of state is.
2. What distro owns and does not own regarding identity / auth / security, and what the fail-closed boundary is.

The dependency rule is `distro -> Kernel` (contract / SDK / command surface only). Kernel owns the communication semantics and contract, and distro-side state is runtime state / product state / evidence state that uses the Kernel contract. distro is not the Kernel semantic authority.
...
SFU / TURN / Signaling distro handle session, participant, allocation, route, relay, subscription, and command evidence. Leaving the state / persistence boundary unfixed causes reference state, product database, benchmark fixture, and evidence artifact to crosstalk, and reference success to be diverted into product persistence readiness.

Kernel owns the communication semantics and contract. distro-side state is runtime state / product state / evidence state that uses the Kernel contract, and it is not the Kernel semantic authority.

### 1.2 State Class Ownership Matrix

| State class | Owner | Persistence rule | Claim boundary |
|---|---|---|---|
| reference runtime state | reference distro | in-memory only | limited to reference behavior |
...
| product runtime state | product distro | requires a product-owned persistence admission record | limited to product scope |
| production database state | product distro / operations | requires a production readiness admission record | not adopted in reference scope |

The reference distro MUST NOT hold a production database. When a product distro adopts a database / queue / external storage, it MUST first record a product persistence admission decision.

### 1.3 Reference State Rule

The state of the reference distro is fixed to in-memory deterministic state only. The reference distro MUST NOT hold a database, queue, external storage, or cloud persistence.
...
Product persistence is owned by `product-distro/persistence-topology/`. Initial product persistence is limited to a topology scaffold, and a concrete database provider is not adopted.
...
SFU / TURN / Signaling distro handle identity, credential, authorization, transport security, TURN credential, and session admission. Leaving the identity / auth / security boundary unfixed causes reference fixture credential, product authentication, live endpoint security, and Kernel reason to crosstalk.

Kernel does not own an auth provider, a tenant identity provider, or a production credential issuer. The distro side also does not issue production auth in the reference scope.
...
| product identity provider | requires a product distro admission record | limited to product scope |
...
The reference distro MUST use deterministic fixture identity only.
...
The product auth provider is not adopted in the initial state. When a product distro adopts an auth provider, it MUST first fix the following.
...
- The reference distro directly holding a database / queue / external storage.
- The reference distro directly adopting a production database.
- Treating a product persistence schema as a Kernel contract.
...
- The reference distro holding a real credential issuer.
- The reference distro holding a production credential issuer.
