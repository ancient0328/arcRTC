# Chapter 07 reference distro design
...
This chapter fixes, at a granularity sufficient for reproducible re-implementation, the structure, responsibility boundaries, per-plane (Signaling / TURN / SFU) distro design, composition design, runtime topology, transport boundary, configuration profile boundary, and composition runtime bridge of the reference distro (the minimal distro layer that consumes the Kernel frozen contract) within the arcRTC v0.2 distro area. This chapter is fully self-contained and is understandable without consulting other documents or source code. Only other chapter numbers within this same specification are referenced.

The reference distro MUST NOT claim production readiness. The reference distro is the minimal distro layer that uses the Kernel frozen contract (contract / SDK / command surface); the dependency direction is `distro -> Kernel` (contract only).

---

## 1. Owned surface of the reference distro

The reference distro owns the following surfaces.

| Surface | Owns |
|---|---|
| `reference-distro/output/` | reference output outcome types; product input subset is the 3-type allow-list (`ReferenceSignalingOutcome` / `ReferenceTurnOutcome` / `ReferenceSfuOutcome`) |
| `reference-distro/signaling/` | Signaling reference wiring |
| `reference-distro/turn/` | TURN reference wiring |
| `reference-distro/sfu/` | SFU reference wiring |
| `reference-distro/composition/` | Signaling / TURN / SFU reference composition |
| `reference-distro/deployment-profiles/` | local / benchmark-oriented profile |
| `reference-distro/ops/` | reference execution helper |

The reference distro MUST NOT own the following.
...
The reference distro packages follow the workspace members. Each package has the following required files.
...
| `reference-distro/deployment-profiles` | `local.toml`, `benchmark.toml` |

Profile files are not Rust workspace members.

### 1.2 Build direction (required direction)

The reference distro MUST be built in the following order.

1. reference output package
2. Signaling reference distro
3. TURN reference distro
4. SFU reference distro
...
What reference distro tests can show is limited to reference behavior and composition behavior. Reference distro tests MUST NOT show production readiness, live readiness, native application readiness, or public distribution readiness.
...
- The reference distro is treated as the same claim as the product distro.
- A reference test pass is adopted as production readiness.
- The reference distro owns product policy, monitoring, or rollback.
- The reference distro modifies the Kernel contract.
- The reference distro holds a database / queue / external storage.
...
## 2. Signaling reference distro design

The Signaling reference distro is the first plane of the reference composition. It provides the connection surface for session / room / participant / control-plane commands but MUST NOT own Kernel core semantics. The owned surface is `reference-distro/signaling/`.
...
```text
reference-distro/signaling -> Kernel public contract
reference-distro/signaling -> Kernel SDK projection
reference-distro/signaling -> distro evidence shape

reference-distro/signaling -X-> Kernel core source modification
reference-distro/signaling -X-> product policy
```

### 2.4 Input / output boundary (execution order)

The Signaling reference distro MUST handle external input in the following order.
...
Signaling reference distro tests MAY show:
...
- The Signaling reference distro reimplements Kernel Signaling semantics.
...
- The Signaling reference distro owns product policy or live endpoint operation.

---

## 3. TURN reference distro design

The TURN reference distro builds the reference runtime surface for allocation / permission / relay. It handles the execution surface of network relay but MUST NOT own Kernel TURN semantics. The owned surface is `reference-distro/turn/`.
...
```text
reference-distro/turn -> Kernel public contract
reference-distro/turn -> Kernel core-owned port definition
reference-distro/turn -> distro evidence shape

reference-distro/turn -X-> Kernel core source modification
reference-distro/turn -X-> product network policy
reference-distro/turn -X-> production readiness claim
```

### 3.4 Packet / relay boundary (execution order)

The TURN reference distro MUST handle packet / relay data in the following order.
...
TURN reference distro tests MAY show:
...
- The TURN reference distro reimplements Kernel allocation / permission / relay semantics.
...
## 4. SFU reference distro design

The SFU reference distro builds the reference runtime surface for media routing / forwarding / quality observation. It handles the execution surface of packet forwarding but MUST NOT own Kernel SFU routing semantics. The owned surface is `reference-distro/sfu/`.
...
```text
reference-distro/sfu -> Kernel public contract
reference-distro/sfu -> Kernel core-owned port definition
reference-distro/sfu -> distro evidence shape

reference-distro/sfu -X-> Kernel core source modification
reference-distro/sfu -X-> product media policy
reference-distro/sfu -X-> live media readiness claim
```

### 4.4 Packet boundary (execution order)

The SFU reference distro MUST handle packet / media input in the following order.
...
Packet payload ownership, buffer lifetime, and zero-copy / copy policy follow the Kernel contract and the driver/runtime boundary. The SFU reference distro MUST NOT redefine packet semantics.

### 4.5 Evidence boundary

SFU reference distro tests MAY show:
...
- The SFU reference distro reimplements Kernel routing / quality semantics.
...
2. Start the Signaling reference distro.
3. Start the TURN reference distro.
4. Start the SFU reference distro.
...
- that the Signaling / TURN / SFU reference distro are connected,
...
|---|---|---|
| `ReferenceCompositionStepInput` | `reference-distro/composition/src/step_input.rs` | `correlation_id: CorrelationId`, `step: ReferenceCompositionStep` |
| `ReferenceCompositionStep` | `reference-distro/composition/src/step_input.rs` | enum variants listed below |
| `ReferenceCompositionPlaneOutcome` | `reference-distro/composition/src/runtime_bridge.rs` | enum variants `Signaling(ReferenceSignalingOutcome)`, `Turn(ReferenceTurnOutcome)`, `Sfu(ReferenceSfuOutcome)`, `Composition` |
| `ReferenceCompositionStepOutcome` | `reference-distro/composition/src/runtime_bridge.rs` | `correlation_id: CorrelationId`, `distro_reason: DistroEvidenceReason`, `applied_plane: DistroPlane`, `plane_outcome: ReferenceCompositionPlaneOutcome`, `non_claim_scope: Vec<DistroNonClaimScope>` |
...
Every failure maps through `ReferenceCompositionError::distro_reason()`.
...
The initial runtime topology of the reference distro is fixed to a single host / controlled process topology.
...
| multi host deployment | handled from product distro readiness onward | not adopted in reference scope |
...
The transport of the reference distro is treated as a staged transport.
...
The reference distro profiles are fixed to the following.
...
| `product-preflight` | product distro preflight | product behavior input | production readiness |
...
The reference profile MUST NOT handle production secrets. When secret-like input is required, it is limited to: deterministic local test token, local fixture credential, non-production key material. Production secret handling is handled in product distro / production readiness scope.
...
1. The dependency direction is `distro -> Kernel` (contract only). The reference distro MUST NOT modify the Kernel contract.
2. Reference success does not mean production readiness / live readiness / benchmark threshold satisfaction (fail-closed).
3. The reference types the product distro can consume are limited to the 3-type allow-list (`ReferenceSignalingOutcome` / `ReferenceTurnOutcome` / `ReferenceSfuOutcome`). See Chapter 08 for details.
4. Every reference error maps to `DistroEvidenceReason` and has no `Unknown`.
