...
本章は、arcRTC v0.2 の distro 領域のアーキテクチャ構造、すなわち root model、4 層構造（distro-support / reference-distro / product-distro / tests）の責務、依存規則、workspace manifest と dependency path 規則、module export 境界（公開 / 非公開の閉集合）、source / package 境界、branch identity 境界、および collapse conditions を、他文書を参照せずに再現実装可能な粒度で確定することを目的とします。本章の規則・語彙・境界はすべて本仕様書内で完結します。

## Root Model

distro 領域の root model は次の構造です。
...
distro は Kernel 外の独立 source root として扱います。Kernel は `Kernel/` に閉じ、distro は Kernel source を変更しません（禁止）。
...
- distro は Kernel public contract / SDK projection / documented command surface へ依存できます（許可）。
- Kernel は distro へ依存しません（禁止／不変条件）。
- distro は Kernel semantic authority を上書きしません（禁止／不変条件）。

## 4 層の責務

### Distro Support Layer

distro support は、reference / product / test が共有する distro-local type を置く層です。

| Surface | 責務 |
|---|---|
| `distro-support/evidence/` | evidence reason / record / validation / common label enum |

distro support は Kernel reason catalog、Kernel semantics、product policy、readiness claim を所有しません（禁止）。

### Reference Distro Layer

reference distro は、Kernel frozen contract を使う最小実装層です。
...
| `reference-distro/composition/` | Signaling / TURN / SFU の結合 |
| `reference-distro/deployment-profiles/` | local / controlled benchmark profile |
| `reference-distro/ops/` | reference execution helper |

reference distro は production readiness を主張しません（禁止）。

### Product Distro Layer

product distro は、`arcrtc-reference-output` の `ReferenceSignalingOutcome` / `ReferenceTurnOutcome` / `ReferenceSfuOutcome` だけを入力として消費し、product policy、deployment、monitoring、persistence topology、rollback を別 surface で所有する層です。

product distro は `ReferenceCompositionOutcome`、reference source、internal state、reference function、success claim を継承しません（禁止）。product distro は reference distro を fork / copy して product policy を混入しません（禁止）。
...
product readiness / production readiness / live readiness は、それぞれ distro 側の evidence と verdict でのみ扱います。
...
| `reference-distro/output/` | reference output outcome types; product input subset は 3-type allow-list | reference state mutation / fixture validation / local auth / runtime execution / composition binding / product policy / readiness claim |
...
- test / benchmark / readiness source を distro source と同一 owner にしない（禁止）。
- Kernel source を distro package に含めない（禁止）。
...
|---|---|
| distro-local type projection | Kernel output を distro-facing evidence shape へ投影する |
...
`reference-distro/deployment-profiles/` と `tests/` 配下は、この workspace member list に含めません（禁止）。test package は `tests/` 配下で別途所有します。
...
すべての distro package から Kernel package への local path は、package directory 起点で `../../../Kernel/...` に固定します。
...
distro-support / reference / product package 間の local path は次に固定します。
...
| `criterion` | benchmark package only; distro package に入れない（禁止） |

`arcrtc-distro-evidence` が canonical evidence serialization owner です。`arcrtc-reference-ops`、`arcrtc-product-monitoring`、`arcrtc-product-deployment`、`arcrtc-product-persistence-topology` の `serde` は、それぞれ config / wrapper / product-local projection struct の serialization に限定し、`DistroEvidenceRecord` の独立再定義に使ってはなりません（禁止）。
...
| `arcrtc-reference-ops` | `error`, `evidence`, `reason`, `runtime` | `arcrtc_distro_evidence` の evidence 型 / ROOT 定数の re-export, `ReferenceRuntimeError`, evidence 系, `distro_reason_closed_set`, runtime 系 |
...
distro の source / test snapshot は、`arcRTC` repository の `distro/` subtree として Git 管理下に公開します。
...
- distro が Kernel core semantics を所有する。
- distro が Kernel port definition を変更する。
- reference distro を production readiness として扱う。
- Kernel evidence を distro readiness proof として扱う。
...
- Kernel source が distro package にコピーされる。
...
- `criterion` を distro package の runtime dependency に入れる。
- `reference-distro/deployment-profiles/` または `tests/` を distro workspace member に入れる。
- `DistroEvidenceReason` を使う package が `arcrtc-distro-evidence` dependency を持たない。
