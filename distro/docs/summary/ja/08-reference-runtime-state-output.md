...
`reference-distro/signaling/src/state.rs` は次の local phase enum を持ちます。
...
`reference-distro/signaling/src/fixture_identity.rs` は次を持ちます。
...
`reference-distro/turn/src/fixture_credential.rs` は次に固定します。
...
`reference-distro/sfu/src/fixture_route_auth.rs` は次に固定します。
...
`reference-distro/sfu/src/state.rs` は次の local phase enum を所有します。
...
`ReferenceSfuAction` は SFU reference state を mutate できる唯一の local input enum です。これらの local SFU state enum は frozen Kernel state variant 名を反映し、distro-only variant を導入してはなりません（MUST NOT）。
...
本節は reference-local API、state field、state transition、Kernel contract builder mapping を固定します。この章は Kernel semantics を再定義しません。distro-local 型は Kernel public contract を作るための入力、projection、evidence に限定します。
...
payload accessor を公開しない Kernel contract object を local source of truth として扱ってはなりません（MUST NOT）。reference behavior application の source は元の distro-local input のままです。

### 3.2 product input allow-list 境界

product distro が消費できる reference 型は `arcrtc-reference-output` が所有する `ReferenceSignalingOutcome`、`ReferenceTurnOutcome`、`ReferenceSfuOutcome` に限定します。
...
| `ReferenceSignalingPayload` | `reference-distro/signaling/src/kernel_contract.rs` | 第1.3節の enum variants |
...
| `ReferenceSfuAction` | `reference-distro/sfu/src/state.rs` | 下記 enum variants |
...
`ReferenceSfuAction` は distro-local です。frozen Kernel `SfuReferenceSet` が `session_id()` のみ public 公開するため、reference state mutation に必要な ID を運びます。variants は次に固定します。
...
| `apply_reference_sfu` | `(&mut ReferenceSfuState, &ReferenceSfuAction) -> Result<ReferenceSfuOutcome, ReferenceSfuError>` | distro-local action field を使い in-memory routing projection を更新 |
...
reference state mutation は Kernel `SfuReferenceSet` から endpoint / stream / route / packet field を読んではなりません。これらは frozen Kernel contract で public accessor ではありません。reference mutation は `ReferenceSfuAction` field を distro-local input として使い、outcome kind のみを Kernel `SfuDecisionKind` に mapping します。
...
各 `Reference*Error` enum は `DistroEvidenceReason` に mapping すること（MUST）。いかなる reference error enum も `Unknown` を含んではなりません（MUST NOT）。

### 3.8 崩壊条件

- Kernel command private payload を distro behavior の source of truth として扱う。
...
- Kernel reason catalog を distro-local reason で拡張する。
...
本節は、product distro が消費できる reference distro surface を `arcrtc-reference-output` package の outcome 型に限定します。product distro は reference source、reference internal state、fixture、local auth、runtime、composition state、reference success claim を継承しません。
...
本節は reference runtime、product runtime、startup、shutdown、drain、correlation propagation の lifecycle API を固定します。runtime lifecycle は distro の process orchestration であり、Kernel semantics を所有しません。runtime lifecycle outcome は command evidence ではありません。build / test command evidence は第6節の runner helper だけが生成します。

### 5.1 runtime states

| Type | File | Variants |
|---|---|---|
| `DistroRuntimeState` | `reference-distro/ops/src/runtime.rs` および `product-distro/deployment/src/runtime.rs` | `Created`, `Starting`, `Running`, `Draining`, `Stopped`, `Failed` |
| `DistroShutdownMode` | `reference-distro/ops/src/runtime.rs` および `product-distro/rollback/src/drain.rs` | `Immediate`, `GracefulLocal`, `DrainThenStop` |
...
| `ReferenceRuntime::state` | `(&self) -> DistroRuntimeState` | local runtime state のみ観測 |
...
runtime timeout と cancellation は distro-local execution facts です。
...
本節は distro command evidence runner、output path、JSON writer、validation command を固定します。command evidence runner は evidence record を生成するだけであり、build success、test pass、benchmark pass、readiness success を自動主張しません。
...
shared evidence record / reason type owner は `arcrtc-distro-evidence` です。distro command root / target root / evidence root constants は `distro-support/evidence/src/validation.rs` が所有し、`arcrtc-distro-evidence` が re-export します。いかなる runner も `distro/target/` 外に evidence を書いてはなりません（MUST NOT）。
...
JSON writer は `DistroEvidenceRecord` を snake_case field 名と所定の wire 値で serialize すること。書き込み前の必須 validation は次です。

- `arcrtc-distro-evidence` の `validate_evidence_record(&record)` が `Ok(())` を返す。
...
- product monitoring evidence helper は command class が `Build` または `Test`、target package が `arcrtc-product-*`、target scope が `product-distro/` 配下である `Product` record のみ受理する。
...
|---|---|---|
| placeholder / forbidden 語句検出の `rg`（`{PROJECT}` / `{MISSION}` / `TODO` / `TBD` / 未定義 等） | `distro` | no output |
| boundary 語句検出の `rg`（`v0.2/userland` / `userland` / `adapters/` / `apps/` 等） | `distro` | no output |
| `UNKNOWN` / `Unknown` / `Other` / `raw String` の `rg` | `distro` | prohibition text のみ |
...
- evidence writer が `target/distro-evidence/` 外に書く。
...
- runner が Kernel working directory command result を distro evidence として採用する。
