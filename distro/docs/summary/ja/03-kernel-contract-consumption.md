...
本章は、arcRTC v0.2 の distro 領域（Kernel 外実装領域）が、frozen 済みの Kernel public contract をどのように import し、version を pin し、reference / product builder に mapping し、consumed Kernel source を凍結 read-only 入力として pin するかを完全に固定します。本章は distro 側が Kernel semantic authority を所有・上書き・複製しないことを不変条件として、import 可能 surface、import path 規則、builder constructor chain の全 mapping、Kernel source pin の必須入力と fail-closed 条件を内在化します。本章は自己完結であり、他文書を開かずに再現実装できる粒度で記述します。

## 依存方向の不変条件

distro は Kernel public contract / SDK projection / documented command surface に依存します（`distro -> Kernel`）。逆方向の依存（`Kernel -> distro`）は禁止です。distro は Kernel semantic authority を所有・上書きしません。Kernel contract の変更が必要になった場合は fail-closed とし、Kernel 側の正式な版管理された契約改定が先行する必要があります。distro 側で Kernel contract を迂回・複製・拡張してはなりません（MUST NOT）。

## 1. Importable surface（利用可能 surface）

distro は次の Kernel surface を利用できます（MAY）。これは閉集合であり、表に列挙されない surface の利用は禁止です。

| Surface | 利用方法 |
|---|---|
| Kernel public contract | SFU / TURN / Signaling distro の入力契約として利用する |
| Kernel SDK projection | Signaling-side public API projection として利用する |
| core-owned port definition | driver / distro boundary の契約として利用する |
...
distro が利用できる Kernel surface の利用方法（consumption boundary）を別表で確定します。

| Kernel surface | distro 側の利用方法 |
|---|---|
| public contract | Signaling / TURN / SFU 実装の入力契約として利用する |
| SDK projection | Signaling public projection として利用する |
| core-owned port definition | distro mapper / runtime wrapper の接続契約として利用する |
| documented command surface | benchmark / test / evidence command の起点として利用する |
| Kernel report reference | non-claim / boundary 根拠として参照する |

## 2. Non-importable surface（利用禁止 surface）

distro は次を import / overwrite / duplicate / 所有してはなりません（MUST NOT）。これは閉集合です。
...
distro は Kernel を次の条件で参照します（MUST）。
...
- Kernel 側の変更が必要な場合、distro では fail-closed とする。

### Upgrade 規則

Kernel dependency を更新する場合、distro 側は次を実施します（MUST）。

1. Kernel 側の正式な版管理された契約改定の有無を確認する。
2. affected distro surface を列挙する。
...
2. distro 側で必要な wrapper / mapper / runtime configuration を定義する。
3. wrapper / mapper は Kernel contract を変換または呼び出すだけに限定する。
4. wrapper / mapper は Kernel semantics を新規定義しない。
5. Kernel contract 不足が判明した場合、distro task を停止する。
6. Kernel 側の正式な版管理された契約改定が存在しない contract は、distro 側で代替定義しない。

### Allowed wrapper（許可 wrapper）

distro 側に置いてよい wrapper は次の閉集合に限定します（MAY）。
...
| error mapper | Kernel reason を distro-facing report reason に投影 |
| command wrapper | benchmark / test / execution command の起動補助 |
| observation mapper | metrics / log / trace output の distro evidence shape への変換 |

### Prohibited wrapper（禁止 wrapper）

distro 側に置いてはならない wrapper は次です（MUST NOT）。
...
- Kernel completion evidence を distro readiness proof に変換する wrapper

## 6. Builder mapping（Kernel public constructor 呼び出し規則）

reference builder が Kernel public constructor をどの順序・どの引数で呼び出すかを固定します。この mapping は Kernel semantics を再定義しません。Kernel source は変更せず、distro は Kernel public API を消費します（MUST）。
...
- command version は初期 reference distro では `CommandVersion::new(1)` に固定する（MUST）。
...
distro は consumed Kernel source snapshot を read-only 入力として pin します。distro 側は Kernel source を変更せず（MUST NOT）、Kernel source を production readiness / live readiness / benchmark threshold satisfaction / real-device success の代替 evidence として扱いません（MUST NOT）。
...
consumed Kernel source snapshot identifier は distro root から次の command で算出します。
...
consumed file count は distro root から次の command で算出します。
...
distro 側は Kernel source を、snapshot digest と file count で識別される read-only かつ version-pinned な入力として消費します。distro 側は consumed Kernel source を distro product completion や readiness evidence に転用してはなりません（MUST NOT）。production readiness / live readiness / benchmark threshold satisfaction / real-device success は distro 側の専用 evidence で成立し、Kernel source pin から導出しません。distro 側の正しい責務は、Kernel source を改変なしに消費し、再実行条件を固定することに限定されます。

## 8. Fail-closed 規則

実装時に Kernel contract の不足が見つかった場合、次の順に扱います（MUST）。

1. distro 側の task を停止する。
2. 不足内容を distro report に記録する。
3. Kernel 側の正式な版管理された契約改定が必要であることを明示する。
4. Kernel contract を distro 側で迂回・複製・拡張しない。
...
- distro 側の fixed-goal 達成作業で Kernel source を変更する。
- Kernel source pin を production readiness / live readiness / benchmark threshold satisfaction / real-device success / distro product completion の代替 evidence にする。

## 9. 不変条件（collapse conditions）

本章が定義する境界は、次のいずれかが発生した時点で崩壊します。これらは禁止であり、発生時は fail-closed とします（MUST NOT）。

- distro が Kernel semantic authority を変更する。
- Kernel contract 不足を distro 側の代替型で補う。
- dependency matrix にない Kernel crate を import する。
- Kernel evidence を distro completion / readiness proof として採用する。
- Kernel source modification を distro の通常 task として扱う。
- distro が Kernel contract を複製・拡張・迂回する。
- wrapper / mapper が Kernel semantics を所有する。
- Kernel contract 不足時に distro task を継続する。
- distro が Kernel source をコピーする。
...
本章は、Kernel final report の再判定、Kernel source の再凍結、production readiness、live readiness、benchmark threshold satisfaction、real-device success、または full fixed-goal completion を単独では主張しません。本章は distro 側の Kernel contract consumption / version pin / builder mapping / Kernel source pin の境界のみを固定します。
