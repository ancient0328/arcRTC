...
本章は、arcRTC v0.2 distro 領域における reference distro（Kernel frozen contract を利用する最小実装層）の構造、責務境界、各 plane（Signaling / TURN / SFU）の distro design、composition design、runtime topology、transport 境界、configuration profile 境界、composition runtime bridge を、再現実装可能な粒度で固定します。本章は完全自己完結であり、他文書・実コードを参照せずに理解できます。同一仕様書内の他章番号のみ参照します。

reference distro は production readiness を主張しません。reference distro は Kernel frozen contract（contract / SDK / command surface）を使う最小実装層であり、依存方向は `distro -> Kernel`（contract のみ）です。

---

## 1. reference distro の所有面

reference distro は次の surface を所有します。

| Surface | 所有内容 |
|---|---|
| `reference-distro/output/` | reference output outcome 型。product input subset は 3-type allow-list（`ReferenceSignalingOutcome` / `ReferenceTurnOutcome` / `ReferenceSfuOutcome`） |
...
| `reference-distro/deployment-profiles/` | local / benchmark 向け profile |
| `reference-distro/ops/` | reference execution helper |

reference distro は次を**所有しません（MUST NOT own）**。
...
reference distro package は workspace members に従います。各 package は次の必須ファイルを持ちます。
...
reference distro は次の順に構築すること（MUST）。
...
reference distro test が示せるものは reference behavior と composition behavior に限定します。reference distro test は production readiness、live readiness、native application readiness、public distribution readiness を示しません（MUST NOT）。

### 1.4 崩壊条件（collapse conditions）

次のいずれかが起きた場合、本章の規範は崩れます（fail-closed の是正対象）。

- reference distro を product distro と同一 claim として扱う。
- reference test pass を production readiness として採用する。
- reference distro が product policy、monitoring、rollback を所有する。
- reference distro が Kernel contract を変更する。
- reference distro が database / queue / external storage を持つ。
...
Signaling reference distro は reference composition の最初の plane です。session / room / participant / control-plane command の接続面を提供しますが、Kernel core semantics を所有しません。所有面は `reference-distro/signaling/` です。
...
Signaling reference distro は外部入力を次の順で扱うこと（MUST）。
...
Signaling reference distro test は次を示し得ます。
...
- Signaling reference distro が Kernel Signaling semantics を再実装する。
- reference-local validation が Kernel accept / reject decision を置き換える。
- Signaling reference result を production readiness として扱う。
- Signaling reference distro が product policy または live endpoint operation を所有する。

---

## 3. TURN reference distro design

TURN reference distro は allocation / permission / relay の reference runtime surface を構築します。network relay の実行面を扱いますが、Kernel TURN semantics を所有しません。所有面は `reference-distro/turn/` です。
...
TURN reference distro は packet / relay data を次の順で扱うこと（MUST）。
...
TURN reference distro test は次を示し得ます。
...
- TURN reference distro が Kernel allocation / permission / relay semantics を再実装する。
...
SFU reference distro は media routing / forwarding / quality observation の reference runtime surface を構築します。packet forwarding の実行面を扱いますが、Kernel SFU routing semantics を所有しません。所有面は `reference-distro/sfu/` です。
...
SFU reference distro は packet / media input を次の順で扱うこと（MUST）。
...
packet payload の所有権、buffer lifetime、zero-copy / copy policy は Kernel contract と driver/runtime boundary に従います。SFU reference distro は packet semantics を再定義しません（MUST NOT）。

### 4.5 evidence 境界

SFU reference distro test は次を示し得ます。
...
- SFU reference distro が Kernel routing / quality semantics を再実装する。
...
reference composition の価値は、3 plane（Signaling / TURN / SFU）を Kernel frozen contract に従って組み合わせ、benchmark / real-device / product 実装の基礎になる composition surface を作ることです。composition は product distro ではなく、production readiness / live readiness を主張しません。所有面は `reference-distro/composition/` です。

### 5.1 所有するもの

| Owned item | 意味 |
|---|---|
| plane wiring | Signaling / TURN / SFU reference distro の接続 |
...
2. Signaling reference distro を起動する。
3. TURN reference distro を起動する。
4. SFU reference distro を起動する。
...
- Signaling / TURN / SFU reference distro が接続されること
...
| `ReferenceCompositionStep` | `reference-distro/composition/src/step_input.rs` | 下記 enum variants |
...
すべての failure は `ReferenceCompositionError::distro_reason()` を通して mapping します。
...
reference distro の初期 runtime topology は single host / controlled process topology に固定します。
...
| multi host deployment | product distro readiness 以降で扱う | reference scope では不採用 |
...
reference distro の transport は staged transport として扱います。
...
reference distro profile を次に固定します。
...
reference profile では production secret を扱いません（MUST NOT）。secret-like input が必要な場合は次に限定します: deterministic local test token、local fixture credential、non-production key material。production secret handling は product distro / production readiness scope で扱います。
...
1. 依存方向は `distro -> Kernel`（contract のみ）。reference distro は Kernel contract を変更しません（MUST NOT）。
2. reference success は production readiness / live readiness / benchmark threshold satisfaction を意味しません（fail-closed）。
3. product distro が消費できる reference 型は 3-type allow-list（`ReferenceSignalingOutcome` / `ReferenceTurnOutcome` / `ReferenceSfuOutcome`）に限定します。詳細は第08章を参照。
4. すべての reference error は `DistroEvidenceReason` に mapping し、`Unknown` を持ちません。
