...
本章は、arcRTC v0.2 implementations 領域（Kernel 外実装領域）における evidence reason の所有、observability evidence reason、error reason mapping、evidence schema、evidence wire format、evidence extension record を、再現実装が可能な粒度で完全に固定します。本章は完全に自己完結しており、本仕様書内の他章番号参照のみを許可し、外部ファイル・Kernel 文書・実コードを開かずに理解および再現実装できます。

依存規則は `implementations -> Kernel`（contract / SDK / command surface のみ）です。implementations は Kernel reason catalog を所有しません。implementations は自領域の evidence reason owner を持ち、Kernel reason は補助的な参照 field として扱います。
...
implementations 全体で共有する evidence reason、evidence record、command class、layer、plane、environment class、non-claim scope の Rust 型所有を本領域内に固定します。これらは reference 領域にも product 領域にも偏らない共有 owner（shared owner）に属します。

この所有は Kernel reason catalog を所有しません。implementations-local evidence reason は Kernel reason を補助的に参照できますが、Kernel reason に昇格しません。
...
`arcrtc-reference-ops` と `arcrtc-product-monitoring` は evidence helper / writer を所有できます。ただし、`ImplementationEvidenceReason` と `ImplementationEvidenceRecord` の型 authority は `arcrtc-implementation-evidence` に置きます。

### 1.3 Required Files

`implementation-support/evidence` は次の file contract を持たなければなりません（MUST）。
...
`implementation-support/evidence/src/lib.rs` は次の export に固定します。
...
`ImplementationEvidenceRecord` は本章 4 節の canonical Rust type と一致しなければなりません（MUST）。`non_claim_scope` は `Vec<ImplementationNonClaimScope>` に固定し、自由文字列 vector として入力してはなりません（MUST NOT）。

`ReadinessClaim`、`ReadinessAdmissionState`、`ReadinessValidationContext`、`ReadinessEvidenceRecord`、`ReadinessEvidenceValidationError`、`validate_readiness_evidence_record` の型 authority は `implementation-support/evidence/src/readiness_extension.rs` に限定します。

`arcrtc-reference-ops/src/evidence.rs` と `arcrtc-product-monitoring/src/evidence.rs` は次のいずれかに限定しなければなりません（MUST）。

1. `arcrtc-implementation-evidence` の型を re-export する。
2. `arcrtc-implementation-evidence` の型を受け取り JSON writer / helper を実装する。

同名の独立 enum / struct を再定義してはなりません（MUST NOT）。

### 1.6 Dependency Rule

`ImplementationEvidenceReason` を field、return type、error mapping に使う package は、必ず `arcrtc-implementation-evidence` に local path dependency を持たなければなりません（MUST）。

`arcrtc-implementation-evidence` は Kernel crate に依存してはなりません（MUST NOT）。Kernel reason を参照する場合は `String` / `Option<String>` field に閉じ、Kernel reason type を support package に取り込んではなりません（MUST NOT）。

### 1.7 Failure Mapping Rule

各 implementations-local error enum の `implementation_reason(&self) -> ImplementationEvidenceReason` は、`arcrtc_implementation_evidence::ImplementationEvidenceReason` を返さなければなりません（MUST）。package-local copy、type alias による別 authority、同名 enum の再定義は禁止します（MUST NOT）。

### 1.8 Serialization Rule

`ImplementationEvidenceRecord` は JSON evidence の canonical serialization source です。closed enum の serialization は本章 5 節の wire format に固定します。

`serde` dependency は、canonical evidence record / closed enum serialization owner としては `arcrtc-implementation-evidence` に限定して導入できます（MAY）。product profile / persistence topology / deployment config などの non-evidence local serialization は、別途許可された package に限ります。`serde_json` による file write は evidence writer owner package に限定します（MUST）。
...
Kernel reason は Kernel semantic authority に属します。implementations は Kernel reason を `kernel_reason` field として参照できますが（MAY）、Kernel reason catalog を追加・変更してはなりません（MUST NOT）。`kernel_reason` は Kernel reason catalog の所有権を implementations に移しません。
...
implementation evidence reason は `implementation_reason` field として扱います。Rust type owner は本章 1 節に従い、`implementation-support/evidence/src/reason.rs` に固定します。
...
| implementation evidence reason | implementations | closed set で定義する |
...
### 2.4 Implementation Evidence Reason 閉集合

implementation evidence reason は次の閉集合（12 項目）に固定します。`UNKNOWN` は正式 evidence reason として禁止します（MUST NOT）。

| Reason | 意味 |
|---|---|
| `IMPLEMENTATION_OK` | 期待された implementation command / behavior が成功した |
| `KERNEL_CONTRACT_UNAVAILABLE` | 必要な Kernel public contract が存在しない |
| `KERNEL_CONTRACT_MISMATCH` | import した Kernel contract shape が本仕様で固定した implementations contract shape と一致しない |
...
| `target_plane` | `ImplementationPlane` closed set（本章 4 節） |
| `kernel_reason` | Kernel reason を参照する場合のみ設定 |
| `implementation_reason` | closed set から設定 |
...
各 implementations-local error enum は次を満たさなければなりません（MUST）。

- `Unknown` / `Other` / raw `String` variant を持たない。
- raw dependency error を public variant にしない。
- `implementation_reason(&self) -> arcrtc_implementation_evidence::ImplementationEvidenceReason` を持つ。
...
`ImplementationEvidenceReason` は `implementation-support/evidence/src/reason.rs` が唯一の canonical Rust enum を持ちます。`arcrtc-reference-ops/src/reason.rs` と `arcrtc-product-monitoring/src/evidence.rs` は再定義せず（MUST NOT）、`arcrtc_implementation_evidence` の型を利用または re-export します。variant set は本章 2.4 の closed set と一致しなければなりません（MUST）。
...
`implementation-support/evidence/src/record.rs` は、次の canonical Rust type を持たなければなりません（MUST）。`arcrtc-reference-ops/src/evidence.rs` と `arcrtc-product-monitoring/src/evidence.rs` は、この型を利用または re-export し、同等の独立型を再定義してはなりません（MUST NOT）。
...
`ImplementationEvidenceReason` は本章 2.4 の closed set であり、本章 1 節が所有します。すべての closed enum の wire serialization は本章 5 節のみが定義します。
...
- `correlation_id` が空。
- `command` が空。
- `working_directory` が `implementations/` の下にない。
- `target_scope` が空。
- `toolchain_runtime_version` が空。
- `expected_outcome` が空。
- `implementation_reason` が closed set にない。
- `implementation_reason` が `UNKNOWN`。
...
`validate_evidence_record(&record)` は本章に列挙した base field のみを validate します。readiness admission reference は `ImplementationEvidenceRecord` の field ではないため、validate しません。`command_class` が `ProductionReadiness` または `LiveReadiness` の base record は、`ReadinessEvidenceRecord` に埋め込まれ本章 6 節の readiness extension validator を通過しない限り diagnostic-only です。

### 4.5 Command-Class Required Field Matrix

`ImplementationEvidenceRecord` は `target_package`、`input_fixture_or_workload`、`exit_status` を JSON shape level で optional に保ちます（すべての evidence class が 3 field を使うわけではないため）。正式 command evidence は採用前に次の command-class matrix を適用しなければなりません（MUST）。matrix cell が `required` の場合、`target_package` と `input_fixture_or_workload` は `Some(non-empty trimmed string)`、`exit_status` は `Some(i32)` でなければなりません。この matrix は `validate_evidence_record(&record)` の一部です。
...
`ImplementationEvidenceReason` は `implementation-support/evidence/src/reason.rs` に次の Rust enum として実装します。
...
次の enum は `implementation-support/evidence/src/record.rs` が所有します。
...
`non_claim_scope` は自由文字列 list ではありません。`ImplementationNonClaimScope` の list です。
...
`implementation-support/evidence/src/validation.rs` は validation error enum を所有します。
...
各 validation error は `ImplementationEvidenceReason::EvidenceFieldsIncomplete` に map します。ただし次は例外です。
...
本節は、benchmark / real-device / readiness evidence が `ImplementationEvidenceRecord` をどう拡張するかを Rust struct と JSON shape で固定します。

Rust には struct inheritance がないため、すべての extension record は `#[serde(flatten)] base: ImplementationEvidenceRecord` を持たなければなりません（MUST）。JSON では base field と extension field が同一 object の top-level field として serialize されます。本節は evidence shape を固定します。benchmark threshold、real-device success、production readiness、live readiness を主張しません。
...
- `logs_metrics_location` が `target/implementation-evidence/real-device/` の外
...
各 readiness validation error は `ImplementationEvidenceReason` に次のように map します。
...
extension evidence writer は、extension JSON を書く前に `validate_evidence_record(&record.base)` と extension-specific validator を呼ばなければなりません（MUST）。base validation が失敗した場合、extension writer は map された `ImplementationEvidenceReason` を返し、JSON を書きません（fail-closed）。readiness の場合、extension-specific validator は `arcrtc-implementation-evidence` の `validate_readiness_evidence_record(&record, &context)` です。context type と admission-state closed set は本章 1 節が所有します。
...
- `ImplementationEvidenceReason` を package ごとに再定義する。
- `ImplementationNonClaimScope` を raw string list として再定義する。
...
- implementation reason が Kernel reason catalog を変更する。
...
- package-local `ImplementationEvidenceReason` enum を再定義する。
...
- Kernel working directory の evidence を implementations evidence として採用する。
- `ImplementationEvidenceRecord` を reference / product package ごとに再定義する。
