...
本章は、arcRTC v0.2 の implementations 領域（Kernel 外実装領域）における実装言語 / toolchain 境界、runtime（executor）境界、workspace 構成、および dependency admission（依存許可）の閉集合を完全に固定します。本章は、SFU / TURN / Signaling の primary implementation 言語、Rust workspace root / member、runtime executor の所有・非所有、Kernel local path dependency matrix、implementations-local dependency matrix、external dependency の許可リスト、admission record の必須フィールド、禁止 pattern、および fail-closed 条件を内在化します。本章は自己完結であり、他文書を開かずに再現実装できる粒度で記述します。

## 依存方向の不変条件

implementations 側 dependency は Kernel contract を消費するための technical dependency であり、Kernel semantics を所有しません（`implementations -> Kernel`、`Kernel -X-> implementations`）。runtime executor も external dependency も、Kernel semantic authority / Kernel reason catalog / Kernel port definition を所有してはなりません（MUST NOT）。dependency API の都合で Kernel semantics を変えてはなりません（MUST NOT）。

## 1. 実装言語 / toolchain 境界

implementations の SFU / TURN / Signaling primary implementation language は Rust に固定します（MUST）。Kernel は Rust workspace として成立しており、implementations 側は Kernel を変更せず、Kernel contract を利用する側の source package として設計します。

| Surface | 決定 | Authority boundary |
|---|---|---|
| reference implementation | Rust | SFU / TURN / Signaling の reference runtime behavior を所有する |
| product implementation | Rust を primary とする | product policy / persistence / deployment は product implementation 側で所有する |
| benchmark harness | Rust primary | Kernel / implementations の comparable workload を実行する |
| real-device harness | 必要時に browser / native / shell harness を追加可能 | real-device command evidence に限定し、implementation authority を所有しない |
| TypeScript / Swift / Kotlin | harness / projection に限定 | SFU / TURN / Signaling implementation owner にしない |

Rust package は `implementations/` 配下に配置します（MUST）。Kernel source を implementations 側へコピーしてはなりません（MUST NOT）。Kernel crate は version / path pin boundary に従って利用します。
...
implementations の Rust workspace root を次に固定します。
...
Kernel workspace へ implementations package を member として追加してはなりません（MUST NOT）。implementations は別 workspace として Kernel local path dependency を利用します。
...
`reference-implementation/deployment-profiles/` は runtime profile data を置く領域であり、初期状態では Rust package にしません。`tests/` 配下の packages は `tests/` 配下で別途所有します。

## 4. Runtime executor

reference / product runtime executor は Tokio-compatible runtime を primary executor として採用します（MUST）。runtime executor は implementations 側 dependency であり、Kernel dependency ではありません。
...
| reference plane | `arcrtc-reference-signaling`, `arcrtc-reference-turn`, `arcrtc-reference-sfu` | `src/kernel_contract.rs`, `src/state.rs`, reference implementation で固定された fixture/auth files, `src/error.rs` |
...
`implementation-support/evidence` は `src/kernel_contract.rs` を持ちません。product support packages も上表に列挙されない限り `src/kernel_contract.rs` を作りません。binary entrypoint は reference / product completion 前には作りません。CLI / command binary が必要な場合は第14章で固定された command surface に従って追加します。
...
| transport | UDP / TCP / WebSocket / TLS crate | transport surface | I/O implementation に限定 |
...
## 8. Allowed implementations-local dependency matrix（閉集合）

implementations-local path dependency は次の範囲に限定します。表に列挙されない implementations-local package への依存は禁止です（MUST NOT）。
...
Kernel package を追加で必要とする場合は、source 変更前にこの matrix を更新します（MUST）。literal local path と package manifest shape は第02章の workspace manifest / dependency path 規則に従います。Test package dependency admission は test workspace manifest が所有し、implementation workspace package の dependency admission と混在させません。product plane package から参照できる reference-facing dependency は `arcrtc-reference-output` のみに限定します。product plane package は `arcrtc-reference-signaling`、`arcrtc-reference-turn`、`arcrtc-reference-sfu`、`arcrtc-reference-composition`、`arcrtc-reference-ops` に依存してはなりません（MUST NOT）。
...
- product persistence provider を reference implementation に追加する。
- cloud provider SDK を reference implementation に追加する。
- dependency version を report 後に成功へ合わせて変更する。
- `ImplementationEvidenceReason` を使う package から `arcrtc-implementation-evidence` dependency を省略する。
...
- TypeScript / Swift / Kotlin / browser harness を SFU / TURN / Signaling primary implementation owner にする。
- Kernel source を implementations 側へコピーする。
- npm / yarn を implementations toolchain として採用する。
...
- `implementation-support/evidence` が Kernel crate dependency を持つ。
...
- `tests/` package を implementation workspace の完了条件に混入する。
...
- implementations-local dependency matrix を超える import を行う。
...
本章は、dependency list finalization、supply-chain readiness、reference implementation completion、product implementation completion、benchmark execution、benchmark threshold satisfaction、real-device command success、production readiness、live readiness、security readiness、または Kernel toolchain の変更を主張しません。本章は implementations 側の language / toolchain / runtime executor / dependency admission の境界のみを固定します。
