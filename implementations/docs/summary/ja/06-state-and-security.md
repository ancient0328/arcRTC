...
本章は、arcRTC v0.2 implementations 領域（Kernel 外実装領域）における state / persistence の境界と、identity / auth / security の境界を、再現実装が可能な粒度で完全に固定します。本章は完全に自己完結しており、本仕様書内の他章番号参照のみを許可し、外部ファイル・Kernel 文書・実コードを開かずに理解および再現実装できます。

本章の中心命題は次の 2 点です。

1. implementations は何を永続化し、何を永続化しないか。state の source-of-truth 規則は何か。
2. implementations は identity / auth / security について何を所有し、何を所有しないか。fail-closed の境界は何か。

依存規則は `implementations -> Kernel`（contract / SDK / command surface のみ）です。Kernel は communication semantics と contract を所有し、implementations 側 state は Kernel contract を利用した runtime state / product state / evidence state です。implementations は Kernel semantic authority ではありません。
...
SFU / TURN / Signaling implementations は session、participant、allocation、route、relay、subscription、command evidence を扱います。state / persistence boundary を未固定にすると、reference state、product database、benchmark fixture、evidence artifact が混線し、reference success が product persistence readiness に転用されます。

Kernel は communication semantics と contract を所有します。implementations 側 state は Kernel contract を利用した runtime state / product state / evidence state であり、Kernel semantic authority ではありません。

### 1.2 State Class 所有マトリクス

| State class | Owner | Persistence rule | Claim boundary |
|---|---|---|---|
| reference runtime state | reference implementation | in-memory only | reference behavior に限定 |
...
| product runtime state | product implementation | product-owned persistence admission record が必要 | product scope に限定 |
| production database state | product implementation / operations | production readiness admission record が必要 | reference scope では不採用 |

reference implementation は production database を持ってはなりません（MUST NOT）。product implementation が database / queue / external storage を採用する場合、まず product persistence admission decision を記録しなければなりません（MUST）。

### 1.3 Reference State Rule

reference implementation の state は in-memory deterministic state のみに固定します。reference implementation は database、queue、external storage、cloud persistence を持ってはなりません（MUST NOT）。
...
product persistence は `product-implementation/persistence-topology/` が所有します。初期 product persistence は topology scaffold に限定し、具体 database provider は未採用とします。
...
SFU / TURN / Signaling implementations は identity、credential、authorization、transport security、TURN credential、session admission を扱います。identity / auth / security boundary を未固定にすると、reference fixture credential、product authentication、live endpoint security、Kernel reason が混線します。

Kernel は auth provider、tenant identity provider、production credential issuer を所有しません。implementations 側も、reference scope では production auth を発行しません。
...
| product identity provider | product implementation admission record が必要 | product scope に限定 |
...
reference implementation は deterministic fixture identity のみを使用しなければなりません（MUST）。
...
product auth provider は初期状態では未採用です。product implementation が auth provider を採用する場合、次を先に固定しなければなりません（MUST）。
...
- reference implementation が database / queue / external storage を直接持つ。
- reference implementation が production database を直接採用する。
- product persistence schema を Kernel contract として扱う。
...
- reference implementation が real credential issuer を持つ。
- reference implementation が production credential issuer を持つ。
