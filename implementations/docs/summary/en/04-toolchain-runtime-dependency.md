...
This chapter fixes completely the implementation language / toolchain boundary, the runtime (executor) boundary, the workspace composition, and the closed set of dependency admission in the implementations area of arcRTC v0.2 (the non-Kernel implementation area). This chapter internalizes the SFU / TURN / Signaling primary implementation language, the Rust workspace root / members, the ownership and non-ownership of the runtime executor, the Kernel local path dependency matrix, the implementations-local dependency matrix, the external dependency allow-list, the required fields of the admission record, the prohibited patterns, and the fail-closed conditions. This chapter is self-contained and is written at a granularity sufficient for re-implementation without opening any other document.

## Dependency-direction invariant

An implementations-side dependency is a technical dependency for consuming the Kernel contract and does not own Kernel semantics (`implementations -> Kernel`, `Kernel -X-> implementations`). Neither the runtime executor nor any external dependency MUST own Kernel semantic authority / Kernel reason catalog / Kernel port definition. Kernel semantics MUST NOT be changed for the convenience of a dependency API.

## 1. Implementation language / toolchain boundary

The SFU / TURN / Signaling primary implementation language of implementations is fixed to Rust (MUST). The Kernel exists as a Rust workspace; the implementations side does not change the Kernel and is designed as a source package that consumes the Kernel contract.

| Surface | Decision | Authority boundary |
|---|---|---|
| reference implementation | Rust | owns the reference runtime behavior of SFU / TURN / Signaling |
| product implementation | Rust as primary | product policy / persistence / deployment is owned on the product implementation side |
| benchmark harness | Rust primary | executes comparable workloads of Kernel / implementations |
| real-device harness | browser / native / shell harness MAY be added when required | limited to real-device command evidence; does not own implementation authority |
| TypeScript / Swift / Kotlin | limited to harness / projection | not made an SFU / TURN / Signaling implementation owner |

Rust packages MUST be placed under `implementations/`. The Kernel source MUST NOT be copied to the implementations side. Kernel crates are used in accordance with the version / path pin boundary.

### Toolchain rule (closed set)

| Toolchain | Rule |
|---|---|
| Rust / Cargo | implementations primary toolchain |
...
The Rust workspace root of implementations is fixed as follows.

| Item | Fixed value |
|---|---|
| working directory | `implementations/` |
| workspace manifest | `implementations/Cargo.toml` |
| edition | `2021` |
| rust-version | `1.96` |
| resolver | `2` |
| license | `Apache-2.0` |
...
An implementations package MUST NOT be added to the Kernel workspace as a member. implementations uses the Kernel local path dependency as a separate workspace.
...
|---|---|---|
| `implementation-support/evidence` | `arcrtc-implementation-evidence` | shared implementation evidence / reason types |
| `reference-implementation/output` | `arcrtc-reference-output` | reference output outcome types; product input subset is the 3-type allow-list |
| `reference-implementation/signaling` | `arcrtc-reference-signaling` | Signaling reference implementation |
| `reference-implementation/turn` | `arcrtc-reference-turn` | TURN reference implementation |
| `reference-implementation/sfu` | `arcrtc-reference-sfu` | SFU reference implementation |
| `reference-implementation/composition` | `arcrtc-reference-composition` | reference composition |
| `reference-implementation/ops` | `arcrtc-reference-ops` | reference execution helper |
| `product-implementation/signaling` | `arcrtc-product-signaling` | product Signaling implementation |
| `product-implementation/turn` | `arcrtc-product-turn` | product TURN implementation |
| `product-implementation/sfu` | `arcrtc-product-sfu` | product SFU implementation |
| `product-implementation/product-policy` | `arcrtc-product-policy` | product policy |
| `product-implementation/persistence-topology` | `arcrtc-product-persistence-topology` | product persistence topology |
| `product-implementation/deployment` | `arcrtc-product-deployment` | product deployment profile |
| `product-implementation/monitoring` | `arcrtc-product-monitoring` | product monitoring / probes |
| `product-implementation/rollback` | `arcrtc-product-rollback` | product rollback / drain |

`reference-implementation/deployment-profiles/` is an area for runtime profile data and is not made a Rust package in the initial state. Packages under `tests/` are owned separately under `tests/`.

## 4. Runtime executor

The reference / product runtime executor adopts a Tokio-compatible runtime as the primary executor (MUST). The runtime executor is an implementations-side dependency, not a Kernel dependency.
...
|---|---|---|
| reference executor | `reference-implementation/ops/src/runtime.rs` | local process orchestration, timer, shutdown signal |
| reference composition runner | `reference-implementation/composition/src/runtime_bridge.rs` | Signaling / TURN / SFU local composition execution |
| product executor | `product-implementation/deployment/src/runtime.rs` | product process profile |
| product shutdown / drain | `product-implementation/rollback/src/drain.rs` | product drain orchestration |
...
|---|---|---|
| implementation support | `arcrtc-implementation-evidence` | `src/reason.rs`, `src/record.rs`, `src/readiness_extension.rs`, `src/validation.rs`, `src/error.rs` |
| reference output | `arcrtc-reference-output` | `src/signaling.rs`, `src/turn.rs`, `src/sfu.rs`, `src/composition.rs`, `src/error.rs` |
| reference plane | `arcrtc-reference-signaling`, `arcrtc-reference-turn`, `arcrtc-reference-sfu` | `src/kernel_contract.rs`, `src/state.rs`, the fixture/auth files fixed for the reference implementation, `src/error.rs` |
...
| owner package | implementations package name |
...
| transport | UDP / TCP / WebSocket / TLS crate | transport surface | limited to I/O implementation |
...
| implementations package | Allowed Kernel packages |
|---|---|
...
## 8. Allowed implementations-local dependency matrix (closed set)

The implementations-local path dependency is limited to the following range. Depending on any implementations-local package not listed in the table is forbidden (MUST NOT).

| implementations package | Allowed implementations-local packages |
|---|---|
| `arcrtc-implementation-evidence` | none |
| `arcrtc-reference-output` | `arcrtc-implementation-evidence` |
| `arcrtc-reference-signaling` | `arcrtc-implementation-evidence`, `arcrtc-reference-output` |
| `arcrtc-reference-turn` | `arcrtc-implementation-evidence`, `arcrtc-reference-output` |
| `arcrtc-reference-sfu` | `arcrtc-implementation-evidence`, `arcrtc-reference-output` |
| `arcrtc-reference-composition` | `arcrtc-implementation-evidence`, `arcrtc-reference-output`, `arcrtc-reference-signaling`, `arcrtc-reference-turn`, `arcrtc-reference-sfu` |
| `arcrtc-reference-ops` | `arcrtc-implementation-evidence`, `arcrtc-reference-composition` |
| `arcrtc-product-signaling` | `arcrtc-implementation-evidence`, `arcrtc-reference-output`, `arcrtc-product-policy` |
| `arcrtc-product-turn` | `arcrtc-implementation-evidence`, `arcrtc-reference-output`, `arcrtc-product-policy` |
| `arcrtc-product-sfu` | `arcrtc-implementation-evidence`, `arcrtc-reference-output`, `arcrtc-product-policy` |
| `arcrtc-product-policy` | `arcrtc-implementation-evidence` |
| `arcrtc-product-persistence-topology` | `arcrtc-implementation-evidence` |
| `arcrtc-product-deployment` | `arcrtc-implementation-evidence`, `arcrtc-product-rollback` |
| `arcrtc-product-monitoring` | `arcrtc-implementation-evidence` |
| `arcrtc-product-rollback` | `arcrtc-implementation-evidence` |

When an additional Kernel package is required, this matrix MUST be updated before any source change. The literal local path and package manifest shape follow the workspace manifest / dependency path rules in Chapter 02. Test package dependency admission is owned by the test workspace manifest and MUST NOT be mixed with the dependency admission of implementation workspace packages. The reference-facing dependency that a product plane package may reference is limited to `arcrtc-reference-output` only. A product plane package MUST NOT depend on `arcrtc-reference-signaling`, `arcrtc-reference-turn`, `arcrtc-reference-sfu`, `arcrtc-reference-composition`, or `arcrtc-reference-ops`.
...
| `tracing` | `0.1` | none | observability | structured implementation log / span | adoption as evidence by itself |
...
- Add a product persistence provider to a reference implementation.
- Add a cloud provider SDK to a reference implementation.
- Change a dependency version to match success after a report.
- Omit the `arcrtc-implementation-evidence` dependency from a package that uses `ImplementationEvidenceReason`.
...
- Add an implementations package to a Kernel workspace member.
- Make a TypeScript / Swift / Kotlin / browser harness the SFU / TURN / Signaling primary implementation owner.
- Copy the Kernel source to the implementations side.
- Adopt npm / yarn as an implementations toolchain.
...
- `implementation-support/evidence` has a Kernel crate dependency.
...
- Mix a `tests/` package into the completion conditions of the implementation workspace.
...
- Perform an import beyond the implementations-local dependency matrix.
...
This chapter does not claim dependency list finalization, supply-chain readiness, reference implementation completion, product implementation completion, benchmark execution, benchmark threshold satisfaction, real-device command success, production readiness, live readiness, security readiness, or a change to the Kernel toolchain. This chapter fixes only the boundary of implementations-side language / toolchain / runtime executor / dependency admission.
