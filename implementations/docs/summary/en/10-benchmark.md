...
This chapter fixes, at a granularity sufficient for reproducible implementation, the benchmark domain of the arcRTC v0.2 implementations domain: the scenario set (all 20 scenarios), workload parameters, Criterion configuration, benchmark command, harness package layout, scenario function naming, measurement boundary, evidence conversion, measurement report schema, comparison row admission schema, comparability boundary (scenario-level / measurement-level / threshold-level), the claim boundary of benchmark / real-device / readiness evidence, the acceptance-threshold rule, and fail-closed rules. This chapter is fully self-contained and is understandable without consulting other documents or source code. It references only other chapter numbers within this same specification.
...
```text
arcrtc-implementation-benchmark-tests
```

The `criterion` dependency is allowed only in this package (MAY). The benchmark package MUST NOT be made an implementation workspace member before it is admitted into the workspace.
...
The benchmark command from the implementations root (`implementations/`) is fixed.
...
```text
target/implementation-evidence/benchmark/
```
...
| `BENCH-016` | `evidence_json_record_write` | reference | ops | build one `ImplementationEvidenceRecord` and serialize JSON |
...
|---|---|
| room / participant / session | `reference-implementation/signaling/src/fixture_identity.rs` |
| TURN credential | `reference-implementation/turn/src/fixture_credential.rs` |
| SFU route admission | `reference-implementation/sfu/src/fixture_route_auth.rs` |
| packet bytes | benchmark-owned stack or vector allocated before measured function |
| profile | `reference-implementation/deployment-profiles/benchmark.toml` |
...
```rust
criterion_group!(implementation_benchmark_scenarios, bench_all_scenarios);
criterion_main!(implementation_benchmark_scenarios);
```
...
pub fn build_benchmark_evidence_record(
    base: ImplementationEvidenceRecord,
...
`BenchmarkEvidenceRecord` extends `ImplementationEvidenceRecord` and MUST include all fields of the measurement report schema. All benchmark scenarios MUST provide a finite positive `throughput_items_per_second`.
...
Each evidence report MUST additionally hold the following common fields: correlation id / command / working directory / target implementation layer / target plane / environment class / toolchain / runtime version / input fixture / workload / expected outcome / actual outcome / reason classification / non-claim scope / rerun condition. reason classification MUST follow the implementation evidence reason closed set. UNKNOWN reason MUST NOT be adopted in evidence.
...
- `implementation_layer` and `target_plane` do not match the scenario table row for `scenario_id`.
...
scenario id / target implementation layer / target plane / workload / environment class / metric / unit / sample count / warmup / aggregation / comparison baseline / threshold value / comparison rule / failure classification / rerun condition / adoption scope / non-claim scope.
...
- the benchmark package is made an implementation workspace member before it is admitted into the workspace.
