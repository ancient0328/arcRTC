...
A readiness evidence record extends the implementation evidence record (the common base record). The extension fields are as follows.
...
`product-implementation/product-policy/src/provider_admission.rs` defines the following.
...
`product-implementation/persistence-topology/src/provider_admission.rs` defines the following.
...
`product-implementation/deployment/src/production_profile.rs` defines the following.

| Item | Required shape |
|---|---|
| `build_product_production_profile` | `() -> ProductRuntimeProfile` |

`ProductHostClass` has `ProductionAdmitted`. `build_product_production_profile()` returns `ProductHostClass::ProductionAdmitted`, `ImplementationEnvironmentClass::ProductionDeferred`, and `public_endpoint_claimed = false`. `select_product_runtime` treats the `ProductionAdmitted` profile as `ImplementationOk`. The production profile MUST NOT claim public endpoint availability.

### 7.5 Production Monitoring Probe (source contract)

`product-implementation/monitoring/src/production_probe.rs` defines the following.
...
`product-implementation/rollback/src/production_operation.rs` defines the following.
...
`product-implementation/deployment/src/live_endpoint.rs` owns the following.
...
`admit_product_live_endpoint(ProductLiveEndpointClass::ControlledPublicEndpoint)` returns `ImplementationOk`. `admit_product_live_endpoint(ProductLiveEndpointClass::NotAdmitted)` returns `ProductRuntimeError::ReadinessNotAdmitted`.

`admit_public_traversal(ProductPublicTraversalClass::ControlledPublicTraversal)` returns `ImplementationOk`. `admit_public_traversal(ProductPublicTraversalClass::NotAdmitted)` returns `ProductRuntimeError::ReadinessNotAdmitted`.
...
`product-implementation/monitoring/src/live_probe.rs` owns the following.

- `ProductLiveMonitoringProbe`
- `build_live_monitoring_probe`

`build_live_monitoring_probe` requires a non-empty `metric_name` and returns `ProductMonitoringError::EvidenceFieldsIncomplete` for empty metrics. A successful live monitoring probe uses `ImplementationOk`.

### 8.3 Rollback Source Contract

`product-implementation/rollback/src/live_operation.rs` owns the following.

- `execute_live_shutdown_drain`
- `execute_live_restore`

`execute_live_shutdown_drain` requires a non-empty plane list and returns `ProductRollbackError::RuntimeExecutorError` for an empty list. A successful live shutdown drain uses `ImplementationOk`. `execute_live_restore` returns `ImplementationOk` when invoked with a correlation id. Live operation source is placed in the rollback owner package and MUST NOT be mixed into deployment / monitoring / plane packages.
...
## 9. Fixed Goal of the implementations Domain

The fixed goal of implementations is to fully and entirely achieve production readiness, live readiness, benchmark threshold satisfaction, real-device success, and Kernel completion / freeze. This fixed goal MUST NOT adopt a pragmatic compromise, a reinterpretation, or a reduction to a limited claim. The frozen Kernel is consumed without implementations-side Kernel source mutation; a Kernel contract insufficiency is fail-closed and requires a Kernel-side versioned authority first.

The fixed goal connects the following independent evidence surfaces.

- The implementations domain uses only admitted Kernel crates and performs no Kernel source mutation.
- The reference implementation executes Signaling / TURN / SFU / composition behavior via the Kernel contract.
- The product implementation executes product Signaling / TURN / SFU / policy / persistence / deployment / monitoring / rollback surfaces without owning Kernel semantics.
...
- Kernel completion / freeze is evidenced without implementations-side Kernel source mutation.
...
- Kernel source modification is used to satisfy an implementations readiness surface.
