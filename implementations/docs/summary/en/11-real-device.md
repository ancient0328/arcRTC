...
```text
arcrtc-implementation-real-device-tests
```
...
|---|---|---|---|---|
| `AndroidPhysical` | `AndroidApiLevel` | `NativeSdkCommand` | `LocalUsb` | non-empty path under `target/implementation-evidence/real-device/` |
| `AndroidEmulator` | `AndroidApiLevel` | `NativeSdkCommand` | `EmulatorLoopback` | non-empty path under `target/implementation-evidence/real-device/` |
| `IosPhysical` | `IosSystemVersion` | `NativeSdkCommand` | `LocalUsb` | non-empty path under `target/implementation-evidence/real-device/` |
| `IosSimulator` | `IosSystemVersion` | `NativeSdkCommand` | `SimulatorLoopback` | non-empty path under `target/implementation-evidence/real-device/` |
| `DesktopBrowser` | `BrowserVersion` | `BrowserWebrtcCommand` | `LocalBrowser` | non-empty path under `target/implementation-evidence/real-device/` |
| `MobileBrowser` | `BrowserVersion` | `BrowserWebrtcCommand` | `MobileBrowserEmulation` | non-empty path under `target/implementation-evidence/real-device/` |
...
Post-execution evidence fields MUST be recorded only after the platform command returns: actual outcome / exit status / implementation reason / `redacted_device_identifier` (matching literal `redacted` or `sha256:<64 lowercase hex characters>` when `exit_status == 0`). actual outcome and redacted device identifier MUST NOT be fabricated during preflight.

## 5. Wrapper CLI shape

The real-device wrapper command from `implementations/` is fixed.
...
| Condition | exit status | implementation reason |
|---|---|---|
| allowed platform command succeeded and evidence fields complete | `0` | `IMPLEMENTATION_OK` |
...
| command working directory outside implementations | `5` | `COMMAND_SCOPE_MISMATCH` |
...
| command working directory outside implementations | `COMMAND_SCOPE_MISMATCH` |
...
- `working_directory` is `implementations`.
...
- `logs_metrics_location` is a non-empty path under `target/implementation-evidence/real-device/`.
...
|---|---|
| report filename | `<YYYYMMDD>-implementations-KPI-011-REAL-DEVICE-SUCCESS-EVIDENCE.md` |
...
- the wrapper working directory is outside `implementations`.
...
- the wrapper command omits the manifest path and depends on implementation workspace membership.
