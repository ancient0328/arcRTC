...
The dependency rule is `implementations -> Kernel` (contract / SDK / command surface only). implementations MUST NOT own the Kernel reason catalog. implementations owns the evidence reason owner of its own domain and treats Kernel reason as an auxiliary reference field.
...
The Rust type ownership of the evidence reason, evidence record, command class, layer, plane, environment class, and non-claim scope shared across implementations is fixed within this domain. These belong to a shared owner that is biased toward neither the reference domain nor the product domain.

This ownership MUST NOT own the Kernel reason catalog. An implementations-local evidence reason MAY reference a Kernel reason as auxiliary, but it MUST NOT be promoted to a Kernel reason.
...
|---|---|---|
| `implementation-support/evidence` | `arcrtc-implementation-evidence` | implementation evidence reason / record / validation / readiness extension / common labels / non-claim scope |

`arcrtc-reference-ops` and `arcrtc-product-monitoring` MAY own evidence helpers / writers. However, the type authority of `ImplementationEvidenceReason` and `ImplementationEvidenceRecord` MUST reside in `arcrtc-implementation-evidence`.

### 1.3 Required Files

`implementation-support/evidence` MUST hold the following file contract.

| File | Role |
|---|---|
| `src/lib.rs` | public export |
| `src/reason.rs` | `ImplementationEvidenceReason` closed enum |
| `src/record.rs` | `ImplementationEvidenceRecord`, closed label enums, `ImplementationNonClaimScope` |
...
`implementation-support/evidence/src/lib.rs` is fixed to the following export.
...
`ImplementationEvidenceRecord` MUST match the canonical Rust type in Section 4 of this chapter. `non_claim_scope` is fixed to `Vec<ImplementationNonClaimScope>` and MUST NOT be entered as a free string vector.

The type authority of `ReadinessClaim`, `ReadinessAdmissionState`, `ReadinessValidationContext`, `ReadinessEvidenceRecord`, `ReadinessEvidenceValidationError`, and `validate_readiness_evidence_record` is limited to `implementation-support/evidence/src/readiness_extension.rs`.
...
1. Re-export the types of `arcrtc-implementation-evidence`.
2. Receive the types of `arcrtc-implementation-evidence` and implement a JSON writer / helper.

They MUST NOT redefine an independent enum / struct of the same name.

### 1.6 Dependency Rule

A package that uses `ImplementationEvidenceReason` in a field, return type, or error mapping MUST hold a local path dependency on `arcrtc-implementation-evidence`.

`arcrtc-implementation-evidence` MUST NOT depend on a Kernel crate. When referencing a Kernel reason, it MUST be confined to a `String` / `Option<String>` field, and the Kernel reason type MUST NOT be imported into the support package.

### 1.7 Failure Mapping Rule

`implementation_reason(&self) -> ImplementationEvidenceReason` on each implementations-local error enum MUST return `arcrtc_implementation_evidence::ImplementationEvidenceReason`. A package-local copy, an alternate authority via type alias, and redefinition of a same-name enum are forbidden (MUST NOT).

### 1.8 Serialization Rule

`ImplementationEvidenceRecord` is the canonical serialization source of JSON evidence. The serialization of closed enums is fixed to the wire format in Section 5 of this chapter.

The `serde` dependency MAY be introduced limited to `arcrtc-implementation-evidence` as the canonical evidence record / closed enum serialization owner. Non-evidence local serialization such as product profile / persistence topology / deployment config is limited to packages separately admitted. File writes via `serde_json` MUST be limited to the evidence writer owner package.
...
Kernel reason belongs to the Kernel semantic authority. implementations MAY reference a Kernel reason as a `kernel_reason` field, but MUST NOT add to or change the Kernel reason catalog. `kernel_reason` does not transfer ownership of the Kernel reason catalog to implementations.
...
The implementation evidence reason is treated as the `implementation_reason` field. The Rust type owner follows Section 1 of this chapter and is fixed to `implementation-support/evidence/src/reason.rs`.
...
| implementation evidence reason | implementations | defined as a closed set |
...
### 2.4 Implementation Evidence Reason Closed Set

The implementation evidence reason is fixed to the following closed set (12 items). `UNKNOWN` is forbidden as an official evidence reason (MUST NOT).

| Reason | Meaning |
|---|---|
| `IMPLEMENTATION_OK` | expected implementation command / behavior succeeded |
| `KERNEL_CONTRACT_UNAVAILABLE` | required Kernel public contract is absent |
| `KERNEL_CONTRACT_MISMATCH` | imported Kernel contract shape does not match the implementations contract shape fixed in this specification |
...
| `implementation_layer` | `ImplementationLayer` wire value closed set: `reference` / `product` / `benchmark` / `real_device` / `readiness` |
| `target_plane` | `ImplementationPlane` closed set (Section 4 of this chapter) |
| `kernel_reason` | set only when referencing a Kernel reason |
| `implementation_reason` | set from the closed set |
...
| Package | File | Role |
|---|---|---|
| `arcrtc-implementation-evidence` | `src/reason.rs` | implementation evidence reason enum owner |
| `arcrtc-implementation-evidence` | `src/record.rs` | common evidence record / label enum owner |
...
Each implementations-local error enum MUST satisfy the following.

- Has no `Unknown` / `Other` / raw `String` variant.
- Does not make a raw dependency error a public variant.
- Has `implementation_reason(&self) -> arcrtc_implementation_evidence::ImplementationEvidenceReason`.
- `Display` is limited to diagnostic text and is not adopted as a claim reason.
- The `std::error::Error` implementation is optional, but implementing it does not replace the evidence reason.

### 3.2 Reference Error Mapping

| Error enum | Required variants | ImplementationEvidenceReason |
|---|---|---|
...
`ImplementationEvidenceReason` holds its single canonical Rust enum in `implementation-support/evidence/src/reason.rs`. `arcrtc-reference-ops/src/reason.rs` and `arcrtc-product-monitoring/src/evidence.rs` MUST NOT redefine it; they use or re-export the type of `arcrtc_implementation_evidence`. The variant set MUST match the closed set in Section 2.4 of this chapter.
...
`implementation-support/evidence/src/record.rs` MUST hold the following canonical Rust type. `arcrtc-reference-ops/src/evidence.rs` and `arcrtc-product-monitoring/src/evidence.rs` use or re-export this type and MUST NOT redefine an equivalent independent type.
...
| Enum | Variants |
|---|---|
| `ImplementationCommandClass` | `Format`, `Build`, `Test`, `Benchmark`, `RealDevice`, `ProductionReadiness`, `LiveReadiness` |
| `ImplementationLayer` | `Reference`, `Product`, `Benchmark`, `RealDevice`, `Readiness` |
| `ImplementationPlane` | `Signaling`, `Turn`, `Sfu`, `Composition`, `Ops`, `Policy`, `Persistence`, `Deployment`, `Monitoring`, `Rollback` |
| `ImplementationEnvironmentClass` | `LocalDocsOnly`, `LocalSingleHost`, `ControlledProcess`, `BenchmarkHost`, `RealDeviceBounded`, `ProductionDeferred`, `LiveDeferred` |
| `ImplementationNonClaimScope` | `SourceImplementationCompletionNotClaimed`, `ProductCompletionNotClaimed`, `TestPassNotClaimed`, `BenchmarkThresholdNotClaimed`, `CommandTargetSuccessNotClaimed`, `BehaviorCorrectnessNotClaimed`, `NativeApplicationReadinessNotClaimed`, `PublicDistributionReadinessNotClaimed`, `ProductionReadinessNotClaimed`, `LiveReadinessNotClaimed`, `KernelCompletionNotClaimed`, `KernelFreezeNotClaimed` |

`ImplementationEvidenceReason` is the closed set in Section 2.4 of this chapter and is owned by Section 1 of this chapter. The wire serialization of all closed enums is defined only by Section 5 of this chapter.
...
- `command` is empty.
- `working_directory` is not under `implementations/`.
- `target_scope` is empty.
- `toolchain_runtime_version` is empty.
- `expected_outcome` is empty.
- `implementation_reason` is not in the closed set.
- `implementation_reason` is `UNKNOWN`.
...
`ImplementationEvidenceReason` is implemented in `implementation-support/evidence/src/reason.rs` as the following Rust enum.
...
| Variant | Wire value |
|---|---|
| `ImplementationOk` | `IMPLEMENTATION_OK` |
...
The following enums are owned by `implementation-support/evidence/src/record.rs`.
...
`non_claim_scope` is not a free string list. It is a list of `ImplementationNonClaimScope`.
...
| Variant | Wire value |
|---|---|
| `SourceImplementationCompletionNotClaimed` | `source_implementation_completion_not_claimed` |
...
`implementation-support/evidence/src/validation.rs` owns the validation error enum.
...
Each validation error maps to `ImplementationEvidenceReason::EvidenceFieldsIncomplete`, except:

| Validation error | Reason |
|---|---|
| `WorkingDirectoryOutsideImplementations` | `CommandScopeMismatch` |
...
This section fixes, with Rust struct and JSON shape, how benchmark / real-device / readiness evidence extends `ImplementationEvidenceRecord`.

Because Rust has no struct inheritance, every extension record MUST hold `#[serde(flatten)] base: ImplementationEvidenceRecord`. In JSON, base fields and extension fields are serialized as top-level fields of the same object. This section fixes the evidence shape. It does not claim benchmark threshold, real-device success, production readiness, or live readiness.
...
- `base.command_class != ImplementationCommandClass::Benchmark`
...
- `layer != base.implementation_layer`
...
- `criterion_group != "implementation_benchmark_scenarios"`
- `sample_size != 100`
- `warm_up_seconds != 3`
...
- `base.command_class != ImplementationCommandClass::RealDevice`
...
- `logs_metrics_location` is outside `target/implementation-evidence/real-device/`
...
Benchmark validation errors map to `ImplementationEvidenceReason` as follows.

| Benchmark validation error | Implementation reason |
|---|---|
...
Real-device validation errors map to `ImplementationEvidenceReason` as follows.

| Real-device validation error | Implementation reason |
|---|---|
| `Base(error)` | mapping for `EvidenceValidationError` (Section 5.5 of this chapter) |
| `CommandClassMismatch` | `EVIDENCE_FIELDS_INCOMPLETE` |
...
Owner file: `implementation-support/evidence/src/readiness_extension.rs`
...
Each readiness validation error maps to `ImplementationEvidenceReason` as follows.

| Readiness validation error | Implementation reason |
|---|---|
| `Base(error)` | mapping for `EvidenceValidationError` (Section 5.5 of this chapter) |
...
Extension evidence writers MUST call `validate_evidence_record(&record.base)` and the extension-specific validator before writing extension JSON. If base validation fails, the extension writer returns the mapped `ImplementationEvidenceReason` and does not write JSON (fail-closed). For readiness, the extension-specific validator is `validate_readiness_evidence_record(&record, &context)` from `arcrtc-implementation-evidence`. The context type and admission-state closed set are owned by Section 1 of this chapter.
...
- Redefining `ImplementationEvidenceReason` per package.
- Redefining `ImplementationNonClaimScope` as a raw string list.
...
- An implementation reason changing the Kernel reason catalog.
...
- Redefining a package-local `ImplementationEvidenceReason` enum.
...
- Adopting evidence from a Kernel working directory as implementations evidence.
- Redefining `ImplementationEvidenceRecord` per reference / product package.
