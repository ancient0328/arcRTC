...
This chapter fixes the complete specification of the test package structure and assertion rules in the arcRTC v0.2 implementations domain. Specifically, it describes, at a granularity sufficient for reproducible implementation: the test package set (all 7 packages with their paths, names, and ownership); all boundary / reference / product / benchmark / real-device / readiness test files with each assertion; the extension validator negative branch rule (all benchmark / real-device / readiness validation errors with expected reasons); the expected failure reason rule; the test package manifest shape; manifest path commands; test dependency admission; each test package's `[dependencies]` literal stanza; the path rule; and the testing scope and non-claim of this domain. This chapter is fully self-contained and is understandable without consulting other documents or source code. It references only other chapter numbers within this same specification.

As a dependency rule, implementations depends on the Kernel only through contract / SDK / command surface. This chapter defines the test implementation shape; it does not claim test pass, benchmark pass, real-device success, production readiness, or live readiness. A close-like claim MUST NOT be made on the basis of test definitions alone.

## 1. Role and Non-Claim of the Testing Domain

This chapter is the definition domain of test evidence on the implementations side and defines the test implementation shape; it MUST NOT claim test implementation completion, test pass, benchmark pass, production readiness, or live readiness.

## 2. Test Package Set (all 7 packages)

The test packages are not implementation workspace members. The test package manifest, dependency admission, and manifest-path command are fixed by sections 8 through 13 of this chapter.

| Package path | Package name | Owns |
|---|---|---|
| `tests/boundary` | `arcrtc-implementation-boundary-tests` | docs / dependency / export boundary tests |
| `tests/reference` | `arcrtc-implementation-reference-tests` | reference behavior tests |
| `tests/product` | `arcrtc-implementation-product-tests` | product boundary tests |
| `tests/benchmark` | `arcrtc-implementation-benchmark-tests` | benchmark scenario tests |
| `tests/real-device` | `arcrtc-implementation-real-device-tests` | real-device command evidence wrapper tests |
| `tests/production-readiness` | `arcrtc-implementation-production-readiness-tests` | production readiness matrix tests |
| `tests/live` | `arcrtc-implementation-live-tests` | live readiness matrix tests |
...
| `tests/boundary/tests/evidence_owner.rs` | shared evidence / reason owner, evidence wire values, non-claim scope closed set, and extension evidence owner files match the implementation evidence and reason ownership rule, the evidence wire format rule, and the evidence extension record rule. |
...
| `tests/reference/tests/error_reason.rs` | every reference error variant maps to a fixed implementation reason. |
...
| `tests/product/tests/rollback.rs` | drain / restore plans return `ImplementationOk` only for admitted local / in-memory paths; empty drain and production/provider-deferred paths return fail-closed implementation reasons without live readiness claim. |
| `tests/product/tests/error_reason.rs` | every product error variant maps to a fixed implementation reason. |
...
Test packages are not implementation workspace members. Test execution is not an implementation completion condition.

Test packages are invoked by explicit manifest path from `implementations/`. No `tests/*` package is added to the implementation workspace root members.
...
`[workspace]` is intentionally empty. It prevents Cargo from treating `tests/*` explicit-manifest packages as unlisted members of the parent `implementations` workspace while preserving the rule that test packages are not implementation workspace members.

## 10. Test Package Commands (all manifest path commands)

| Package path | Package name | Command from implementations root |
|---|---|---|
| `tests/boundary` | `arcrtc-implementation-boundary-tests` | `cargo test --manifest-path tests/boundary/Cargo.toml` |
| `tests/reference` | `arcrtc-implementation-reference-tests` | `cargo test --manifest-path tests/reference/Cargo.toml` |
| `tests/product` | `arcrtc-implementation-product-tests` | `cargo test --manifest-path tests/product/Cargo.toml` |
| `tests/benchmark` | `arcrtc-implementation-benchmark-tests` | `cargo test --manifest-path tests/benchmark/Cargo.toml` and `cargo bench --manifest-path tests/benchmark/Cargo.toml --bench benchmark_scenarios` |
| `tests/real-device` | `arcrtc-implementation-real-device-tests` | `cargo run --manifest-path tests/real-device/Cargo.toml -- <platform> --profile reference-local --device-class <device-class>` |
| `tests/production-readiness` | `arcrtc-implementation-production-readiness-tests` | `cargo test --manifest-path tests/production-readiness/Cargo.toml` |
| `tests/live` | `arcrtc-implementation-live-tests` | `cargo test --manifest-path tests/live/Cargo.toml` |

## 11. Test Dependency Admission

| Test package | Required local dependencies |
|---|---|
| `arcrtc-implementation-boundary-tests` | `arcrtc-implementation-evidence` |
| `arcrtc-implementation-reference-tests` | `arcrtc-core-identity`, `arcrtc-core-signaling`, `arcrtc-core-turn`, `arcrtc-core-sfu`, `arcrtc-implementation-evidence`, all reference implementation packages |
| `arcrtc-implementation-product-tests` | `arcrtc-core-identity`, `arcrtc-core-sfu`, `arcrtc-core-signaling`, `arcrtc-core-turn`, `arcrtc-implementation-evidence`, `arcrtc-reference-output`, all product implementation packages |
| `arcrtc-implementation-benchmark-tests` | `arcrtc-core-identity`, `arcrtc-core-sfu`, `arcrtc-core-signaling`, `arcrtc-core-turn`, `arcrtc-implementation-evidence`, `arcrtc-reference-composition`, `arcrtc-reference-ops`, `arcrtc-reference-sfu`, `arcrtc-reference-signaling`, `arcrtc-reference-turn`, `arcrtc-product-deployment`, `arcrtc-product-persistence-topology`, `arcrtc-product-policy`, `arcrtc-product-rollback` |
| `arcrtc-implementation-real-device-tests` | `arcrtc-implementation-evidence` |
| `arcrtc-implementation-production-readiness-tests` | `arcrtc-core-identity`, `arcrtc-implementation-evidence`, product policy, product persistence topology, product monitoring, product rollback, product deployment |
| `arcrtc-implementation-live-tests` | `arcrtc-implementation-evidence`, product monitoring, product rollback, product deployment |

`criterion` is admitted only in `arcrtc-implementation-benchmark-tests`. `serde_json` is admitted in test packages only when evidence JSON parsing / writing is part of the assertion.

## 12. Path Rule

Local path dependencies from `tests/<package>` to implementation packages use `../../`.

Example:

```toml
arcrtc-implementation-evidence = { path = "../../implementation-support/evidence" }
```
...
arcrtc-implementation-evidence = { path = "../../implementation-support/evidence" }
arcrtc-reference-composition = { path = "../../reference-implementation/composition" }
arcrtc-reference-ops = { path = "../../reference-implementation/ops" }
arcrtc-reference-sfu = { path = "../../reference-implementation/sfu" }
arcrtc-reference-signaling = { path = "../../reference-implementation/signaling" }
arcrtc-reference-turn = { path = "../../reference-implementation/turn" }
```
...
arcrtc-core-turn = { path = "../../../Kernel/core/turn" }
arcrtc-implementation-evidence = { path = "../../implementation-support/evidence" }
arcrtc-reference-output = { path = "../../reference-implementation/output" }
arcrtc-product-deployment = { path = "../../product-implementation/deployment" }
arcrtc-product-monitoring = { path = "../../product-implementation/monitoring" }
arcrtc-product-persistence-topology = { path = "../../product-implementation/persistence-topology" }
arcrtc-product-policy = { path = "../../product-implementation/product-policy" }
arcrtc-product-rollback = { path = "../../product-implementation/rollback" }
arcrtc-product-sfu = { path = "../../product-implementation/sfu" }
arcrtc-product-signaling = { path = "../../product-implementation/signaling" }
arcrtc-product-turn = { path = "../../product-implementation/turn" }
```
...
- a test package is counted as implementation package completion.
...
- a `tests/*` package is added to the implementation workspace root members.
...
- a test pass is treated as an implementation completion / readiness claim.
