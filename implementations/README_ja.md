...
arcRTC v0.2 implementations は、凍結済みの arcRTC v0.2 Kernel contract の上に、SFU / TURN / Signaling の reference implementation と product implementation を構築する Kernel 外の領域です。
...
implementations は、凍結済み Kernel contract を、実際に動く reference の通信経路へと具体化する層です。reference の Signaling / TURN / SFU 経路、composition example、benchmark harness を通じて、Kernel が実装・実行・測定できることを示します。要するに、Kernel の実装可能性に関する構成的な証跡です。

- **reference implementation** — Kernel contract が動く Signaling / TURN / SFU 経路として成立することを示す。
- **benchmark** — その振る舞いを再現可能・測定可能にする。
- **product implementation** — 将来の製品化トラックであり、その存在は readiness 主張ではない。

reference implementation は product implementation ではなく、benchmark 結果は production-readiness の主張ではありません。現在の公開範囲は実験的・reference 中心であり、production readiness・clinical / regulated deployment・managed RTC SDK の置換は主張しません。

## 概要

implementations は、Kernel の外側にあって Kernel に依存する4層で構成されます。

- `implementation-support/` — reference / product / tests が共有する implementation-local な evidence / reason 型。
- `reference-implementation/` — 凍結 Kernel contract 上に構築する最小の SFU / TURN / Signaling 実装（signaling / turn / sfu / output / composition / deployment-profiles / ops）。
- `product-implementation/` — reference の output allow-list のみを入力に消費する product runtime。product policy / persistence topology / deployment / monitoring / rollback を所有。
...
Kernel を凍結したまま実装を進化させることで、通信意味論を侵食せずに実装を発展させられます。implementations は、その凍結 contract 上で動く SFU / TURN / Signaling スタックを構築し、特定の Kernel source snapshot に pin し、本番関連の関心事（policy・deployment・monitoring・rollback）を reference 挙動から明確に分離するために存在します。
...
このディレクトリ（`implementations/`）で Rust workspace の確認を実行します。
...
これらのコマンドは implementations scope だけを確認します。

## アーキテクチャモデル

| 層 | 責務 | 境界 |
|---|---|---|
| implementation-support | reference / product / tests が共有する evidence / reason 型 | Kernel semantics・product policy・readiness claim は所有しない |
| reference-implementation | 凍結 Kernel contract 上の最小 SFU / TURN / Signaling 実装 | production readiness を主張しない |
| product-implementation | product runtime・policy・persistence・deployment・monitoring・rollback | reference の output allow-list のみ消費し、reference を product policy に fork しない |
...
| product / benchmark / real-device | product implementation、benchmark、real-device | [09〜11](docs/summary/ja/00-INDEX.md) |
...
- reference / product implementation の完成、
