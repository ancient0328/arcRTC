...
fn rollback_and_restore_plans_do_not_claim_live_readiness() {
    let drain = plan_drain(
        cid("rollback-drain"),
        vec![ImplementationPlane::Signaling, ImplementationPlane::Sfu],
    ...
    assert_eq!(
        empty_drain.implementation_reason,
...
fn kpi_product_rollback_executes_operation_plan_without_live_claim() {
    for path in [
        "product-implementation/rollback/src/drain.rs",
        "product-implementation/rollback/src/restore.rs",
    ] {
        let source =
    ...
    assert_eq!(
        controlled_drain.implementation_reason,
        ImplementationEvidenceReason::ImplementationOk
    );
    ...
    assert_eq!(
        production_deferred.implementation_reason,
        ImplementationEvidenceReason::ReadinessNotAdmitted
    );
    ...
    assert_eq!(
        evidence_restore.implementation_reason,
        ImplementationEvidenceReason::ImplementationOk
    );
