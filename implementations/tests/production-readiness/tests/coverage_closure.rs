...
fn prd_record(gate_id: &str, required_ref: &str) -> ReadinessEvidenceRecord {
    let mut record = ReadinessEvidenceRecord {
        base: base(ImplementationCommandClass::ProductionReadiness),
...
fn production_readiness_base_validation_covers_empty_required_fields() {
    for (record, expected) in [
        {
            let mut record = base(ImplementationCommandClass::ProductionReadiness);
...
fn production_readiness_extension_covers_ref_and_authority_fail_closed_branches() {
    ...
    command_mismatch.base.command_class = ImplementationCommandClass::LiveReadiness;
    assert_eq!(
...
fn production_readiness_drain_and_restore_plans_cover_deferred_reason_branches() {
    ...
    assert_eq!(
        empty_plan.implementation_reason,
    ...
    assert_eq!(
        deferred_plan.implementation_reason,
    ...
    assert_eq!(
        provider_restore.implementation_reason,
        ImplementationEvidenceReason::ReadinessNotAdmitted
    );
    ...
    assert_eq!(
        in_memory_restore.implementation_reason,
...
fn production_readiness_operation_wrappers_cover_error_mappings() {
    ...
    assert_eq!(
        production_drain.implementation_reason,
        ImplementationEvidenceReason::ImplementationOk
    );
    ...
    assert_eq!(
        production_restore.implementation_reason,
        ImplementationEvidenceReason::ImplementationOk
    );
}
