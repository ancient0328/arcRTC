//! implementations dependency admission と workspace 境界を検査します。
...
#[test]
fn implementations_is_not_a_kernel_workspace_member() {
    let kernel_workspace = fs::read_to_string(root().join("../Kernel/Cargo.toml"))
        .expect("Kernel workspace manifest must be readable");
    assert!(
        !kernel_workspace.contains("implementations"),
        "Kernel workspace must not include implementations members"
    );
}
...
fn workspace_members_exclude_tests_and_profile_data() {
    ...
    assert!(!manifest.contains("reference-implementation/deployment-profiles"));
}

#[test]
fn product_plane_manifests_depend_only_on_reference_output() {
    for path in [
        "product-implementation/signaling/Cargo.toml",
        "product-implementation/turn/Cargo.toml",
        "product-implementation/sfu/Cargo.toml",
    ] {
        let body = read(path);
...
#[test]
fn implementation_packages_do_not_admit_criterion_dependency() {
...
fn kernel_production_implementability_kernel_imports_are_allow_listed() {
    for manifest_path in collect_cargo_toml(root()) {
        let relative_path = manifest_path
            .strip_prefix(root())
            .expect("manifest must be under implementations root")
            .to_string_lossy()
            .into_owned();
