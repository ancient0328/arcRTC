...
use arcrtc_implementation_benchmark_tests::{scenario_by_id, workload_id_for_scenario};
...
struct ComparisonRow {
    ...
    warmup_rule: String,
    timestamp: String,
    non_claim_scope: Vec<ImplementationNonClaimScope>,
}
...
fn valid_row() -> ComparisonRow {
    ComparisonRow {
        ...
        non_claim_scope: vec![ImplementationNonClaimScope::BenchmarkThresholdNotClaimed],
    }
}

fn validate_row(row: &ComparisonRow) -> Result<(), &'static str> {
    let scenario = scenario_by_id(&row.scenario_id).ok_or("scenario")?;
    if row.implementation_layer != scenario.layer || row.target_plane != scenario.plane {
...
fn kpi_benchmark_comparison_row_rejects_threshold_claim_without_admission() {
    ...
    wrong_layer.implementation_layer = ImplementationLayer::Product;
    assert_eq!(validate_row(&wrong_layer), Err("layer-plane"));

    let mut wrong_plane = valid_row();
    wrong_plane.target_plane = ImplementationPlane::Turn;
