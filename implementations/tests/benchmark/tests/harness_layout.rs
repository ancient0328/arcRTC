...
fn benchmark_harness_declares_single_canonical_criterion_group() {
    ...
    assert!(source
        .contains("criterion_group!(implementation_benchmark_scenarios, bench_all_scenarios);"));
    assert!(source.contains("criterion_main!(implementation_benchmark_scenarios);"));
