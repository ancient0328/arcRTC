...
/// 現在の working directory が implementations root かを検査します。
pub fn is_current_working_directory_implementations_root() -> bool {
    std::env::current_dir()
        .ok()
        .is_some_and(|path| path.ends_with(IMPLEMENTATIONS_COMMAND_ROOT))
}
...
pub const fn implementation_reason_for_exit(
...
pub const fn actual_outcome_for_exit(planned_exit: RealDeviceWrapperExit) -> &'static str {
    match planned_exit {
        ...
        RealDeviceWrapperExit::CommandScopeMismatch => {
            "command working directory outside implementations"
        }
    }
}
...
pub fn build_real_device_record(
    ...
    RealDeviceEvidenceRecord {
        base: ImplementationEvidenceRecord {
            ...
            kernel_reason: None,
            implementation_reason: implementation_reason_for_exit(planned_exit),
            non_claim_scope: vec![
        ...
        logs_metrics_location: format!("{IMPLEMENTATIONS_EVIDENCE_ROOT}/real-device/wrapper.json"),
...
pub fn build_kpi_real_device_success_record(
    ...
    RealDeviceEvidenceRecord {
        base: ImplementationEvidenceRecord {
            ...
            exit_status: Some(exit_status),
            kernel_reason: None,
            implementation_reason: command_output.exit_status.implementation_reason(),
            non_claim_scope: vec![
                ImplementationNonClaimScope::NativeApplicationReadinessNotClaimed,
                ImplementationNonClaimScope::PublicDistributionReadinessNotClaimed,
                ImplementationNonClaimScope::ProductionReadinessNotClaimed,
        ...
        logs_metrics_location: format!(
            "{IMPLEMENTATIONS_EVIDENCE_ROOT}/real-device/{}.json",
