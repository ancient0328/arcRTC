...
use arcrtc_implementation_evidence::{ImplementationEvidenceReason, ImplementationNonClaimScope};
...
use wrapper::{
    ...
    implementation_reason_for_exit, is_current_working_directory_implementations_root,
...
fn run_wrapper(platform: &str, device_class: &str, profile: &str) -> std::process::Output {
    Command::new(env!(
        "CARGO_BIN_EXE_arcrtc-implementation-real-device-tests"
    ))
    .current_dir(implementations_root())
    .args([
        platform,
        "--profile",
        profile,
...
fn wrapper_exit_mapping_separates_success_platform_unavailable_and_validation_failure() {
    ...
    let _ = is_current_working_directory_implementations_root();
    ...
    assert_eq!(
        implementation_reason_for_exit(RealDeviceWrapperExit::EvidenceValidationFailure),
    ...
    assert_eq!(
        implementation_reason_for_exit(RealDeviceWrapperExit::CommandScopeMismatch),
        ImplementationEvidenceReason::CommandScopeMismatch
    );
}

#[test]
fn wrapper_binary_exits_five_outside_implementations_root() {
...
fn wrapper_binary_reports_platform_command_unavailable_for_missing_adb() {
    ...
    assert!(json["redacted_device_identifier"].is_null());
    assert_eq!(
        json["implementation_reason"],
        serde_json::to_value(ImplementationEvidenceReason::RuntimeExecutorError)
